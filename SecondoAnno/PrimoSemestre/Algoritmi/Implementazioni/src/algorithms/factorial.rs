...
#[test]
fn base_case() {
    assert_eq!(factorial(0, &ProgressBar::hidden()), BigInt::from(1));
}

#[test]
fn negative() {
    assert_eq!(factorial(-3,&ProgressBar::hidden()), BigInt::from(-6));
}
...
#[test]
fn big_input() {
    assert_eq!(factorial(14,&ProgressBar::hidden()), BigInt::from(87178291200_i64));
}
