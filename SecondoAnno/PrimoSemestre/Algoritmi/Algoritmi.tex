...
\begin{enumerate}
  ...
  \item Sistema l'eventuale elemento in più
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Se \( i<k \) cerca a sinistra l'elemento \( i \), altrimenti cerca a destra
    ...
    \] 
    Supponiamo \( T(n) \le cn \):
    \[
      \begin{aligned}
        T(n) &\le c'n + c \left\lceil \frac{n}{5} \right\rceil + c \left( \frac{7}{10}
        n + 6\right) \\
             & \le c'n + c \frac{n}{5} + c + c \left( \frac{7}{10} n + 6 \right) \\
             &= c \frac{7}{10}n + \frac{c}{5}n + 7c + c'n \\
             &= cn - \left( \frac{3}{10}cn - \frac{c}{5}n - 7c - c'n \right) \\
             & \stackrel{?}{\le} cn 
      \end{aligned}
    \] 
    \[
      \begin{aligned}
        \frac{3}{10}cn - \frac{1}{5}cn - 7c - c'n &\ge 0\\
        \frac{1}{10}cn - c'n - 7c &\ge 0\\
        \frac{1}{10}c &> c'\\
        c &> 10c'
      \end{aligned}
    \] 
    Quindi \( T(n) \le cn \) e quindi \( T(n) = O(n) \). Il problema è che le
    costanti sono così alte che nella pratica è meglio il \texttt{randomized\_select}.
\end{enumerate}
