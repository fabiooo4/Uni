...
% Info esame:
% Possibilità di orale obbligatorio in caso di quasi sufficienza
% L'orale può essere chiesto dallo studente come integrazione allo scritto
% solo se lo scritto è sufficiente
%
% BISOGNA RISPONDERE OBBLIGATORIAMENTE A TUTTE LE DOMANDE TEORICHE, (anche in modo
% incompleto) ALTRIMENTI L'ESAME VIENE ANNULLATO
%
% Il preappello è un'alternativa, non un'aggiunta, se lo si fa si ha un tentativo in
% meno ad uno degli appelli ufficiali
%
% Per partecipare al preappello bisogna iscriversi all'appello del 3 febbraio 2025 su
% esse3 che apre il 3 gennaio
%
% L'unica fonta ufficiale per gli appunti è il libro di testo, (si possono saltare
% gli argomenti non trattati in questi appunti)
...
\begin{enumerate}
  \item Questo indirizzo viene assegnato agli host che non hanno ancora un indirizzo IP
    \[
      0.0.0.0
    \] 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{livello2}
...
\begin{example}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{aloha}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{collisioni}
\end{figure}
\noindent
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \draw[->] (0,0) -- (8,0) node[below] {\# Trasmissioni};
    \draw[->] (0,0) -- (0,5) node[above,align=center] {\# Trasmissioni\\con successo};

    \draw[domain=0.2635372:8,smooth,red,thick] plot ({\x},{1.7*exp(-0.5*(\x-1.602))-exp(-(\x-1.602))+0.5})
      node[right] {Aloha};
    \draw[dashed,red,thick] (1.93004,1.2225) -- (0,1.2225) node[left] {19\%};

    \draw[domain=0:5,smooth,dashed] plot ({\x},{\x}) node[above right] {Ideale};
  \end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{slotted-aloha}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \def\b{0.5}
    \def\c{1.602}
    \draw[domain=0.2635372:8,smooth,red] plot ({\x},{\a*exp(-\b*(\x-\c))-exp(-(\x-\c))+0.5})
      node[right] {Aloha};
    \draw[dashed,red] (1.93004,1.2225) -- (0,1.2225) node[left] {19\%};

    \def\a{2.7}
    \def\b{0.3}
    \def\c{2.5}
    \draw[domain=0.924984:8,smooth,blue,thick] plot ({\x},{\a*exp(-\b*(\x-\c))-exp(-(\x-\c))+0.5})
      node[right] {Slotted-Aloha};
    \draw[dashed,blue,thick] (2.80103,2.226796) -- (0,2.226796) node[left] {38\%};

    \draw[domain=0:5,smooth,dashed] plot ({\x},{\x}) node[above right] {Ideale};
  \end{tikzpicture}
  \caption{Efficienza di Slotted-Aloha}
\end{figure}
...
\begin{example}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{csma}
...
\begin{example}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{csma-non-persistent}
...
\begin{example}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{csma-p-persistent}
...
\begin{example}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{csma-cd}
    \caption{Esempio di protocollo CSMA-CD}
  \end{figure}
\end{example}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \draw[->] (0,0) -- (8,0) node[below] {\# Trasmissioni};
    \draw[->] (0,0) -- (0,5.5) node[above,align=center] {\# Trasmissioni\\con successo};
    ...
    \def\b{0.3}
    \def\c{2.5}
    \draw[domain=0.924984:8,smooth,blue] plot ({\x},{\a*exp(-\b*(\x-\c))-exp(-(\x-\c))+0.5})
      node[right] {Slotted-Aloha};
    \draw[dashed,blue] (2.80103,2.226796) -- (0,2.226796) node[left] {38\%};

    \def\a{3.4}
    \def\b{0.1}
    \def\c{3}
    \draw[domain=1.50773:8,smooth,green!50!black,thick] plot ({\x},{\a*exp(-\b*(\x-\c))-exp(-(\x-\c))+0.5})
      node[right,align=center] {CSMA\\Persistent};
    \draw[dashed,green!50!black,thick] (4.19868,3.21434) -- (0,3.21434) node[left] {58\%};

    \def\a{4.5}
    \def\b{0}
    \def\c{4.1}
    \draw[domain=2.490562:8,smooth,purple,thick] plot ({\x},{\a*exp(-\b*(\x-\c))-exp(-(\x-\c))+0.5})
      node[right,align=center] {CSMA\\P-Persistent\\\( (P = 0.01) \) };
    \draw[dashed,purple,thick] (8,5) -- (0,5) node[left] {90\%};

    \draw[domain=0:5,smooth,dashed] plot ({\x},{\x}) node[above right] {Ideale};
  \end{tikzpicture}
  \caption{Efficienza di CSMA}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{ritardo-di-propagazione}
  \includegraphics[width=0.7\textwidth]{vulnerabilita}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    [
      every node/.style={draw,minimum size=1cm,node distance=0cm},
    ]
    \node (p) {Preambolo} node[above,scale=0.8,draw=none] at (p.north) {7 byte};
    \node[right=of p] (f) {Flag} node[above,scale=0.8,draw=none] at (f.north) {1 byte};
    \node[right=of f] (dst) {HW Dest} node[above,scale=0.8,draw=none] at (dst.north) {6 byte};
    \node[right=of dst] (src) {HW Sorg} node[above,scale=0.8,draw=none] at (src.north) {6 byte};
    \node[right=of src] (len) {Payload len} node[above,scale=0.8,draw=none] at (len.north) {2 byte};
    \node[right=of len,text=black,text opacity=1,fill,fill opacity=0.2] (dat) {Payload};
    \node[right=of dat] (check) {Checksum} node[above,scale=0.8,draw=none] at (check.north) {4 byte};
  \end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{indirizzi-mac}
...
\begin{enumerate}
  \item L'host manda in broadcast (di livello 2) una richiesta ARP contenente
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.6\textwidth]{arp-request}
  ...
  \item Solo chi possiede l'indirizzo IP indicato nella richiesta risponde direttamente
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.6\textwidth]{arp-response}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{lan-estesa}
  \caption{LAN estesa}
\end{figure}
\noindent
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{edificio}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{access-point}
          ...
          \item \label{csma-ca4} Se il canale rimane libero per un intervallo DIFS
            \begin{itemize}
              \item 
                ...
                \( s \) è il numero di slot (SIFS) che devo attendere prima di poter
                ritrasmettere
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{csma-ca}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{range}
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \draw[->] (0,0) -- (8,0) node[below] {Spazio};
    \draw[->] (0,0) -- (0,3) node[above,align=center] {Potenza al\\ricevitore};

    \draw[domain=0:8,smooth,blue,samples=300] plot ({\x},{-sqrt(abs((\x - 4)/2)) + 2});

    \draw[red,dashed] (0,0.8) -- (8,0.8) node[right,align=center] {Segnale non\\ricostruibile};
    \fill (4,2) circle (0.1) node[above] {AP};

    \draw[green!50!black,dashed] (1.12,0.8) -- (1.12,0);
    \draw[green!50!black,dashed] (6.88,0.8) -- (6.88,0);
    \draw[green!50!black] (1.12,0) -- ++(0,-0.2) -- (6.88,-0.2)
      node[midway,below] {Range di trasmissione} -- ++(0,0.2);
  \end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{terminale-nascosto}
...
\begin{enumerate}
  ...
  \item Limitare lo spazio di responsabilità dell'access point
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.9\textwidth]{limite-ap}
  ...
  \item Una soluzione protocollare introduce lo scambio di 2 messaggi:
    \begin{itemize}
      ...
      \item \textbf{CTS} (Clear To Send): L'access point manda in \textbf{broadcast} 
        ...
        ricezione impostando un campo chiamato NAV (Network Allocation Vector) 
        con la lunghezza della trama ricevuta dal messaggio CTS. Il NAV può essere
        visto come un contatore che decrementa in modo lineare e quando diventa 0
        la stazione riapre il canale di ricezione.
    \end{itemize}
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.9\textwidth]{rts-cts}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{fisico}
...
\begin{itemize}
  \item Introdurre intervalli temporali tra le trame
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.8\textwidth]{separazione-trame-silenzio}
