...
public class MainEsame {
	...
	public static void main(String[] args) throws StudenteIllegaleException {
		...
		// e lo iscrive all'esame; se si verifica una StudenteIllegaleException, allora
		// ripete
		// la lettura dello studente e il tentativo di iscrizione, ad oltranza;
		// notate che per leggere uno studente da uno scanner esiste un metodo già fatto
		// in Studente
		try (Scanner keyboard = new Scanner(System.in)) {
			while (true) {
				try {
					...
					System.out.println("Dati dello studente errati, riprova:\n" + e.getMessage() + "\n");
				}
			}
		}
		...
		// chiamate perOgniIscritto per stampare le matricole degli studenti fuori corso
		// iscritti all'esame
		esame.perOgniIscritto(s -> s.fuoriCorso(informatica), s -> System.out.println(s.getMatricola()));

		System.out.println("\nStudenti lavoratori:");
		// chiamate perOgniIscritto per stampare gli studenti lavoratori iscritti
		// all'esame
		esame.perOgniIscritto(s -> s instanceof StudenteLavoratore, s -> System.out.println(s));
	}
}