...
\begin{equation}
  \sum_{i=o}^{n} a_i \frac{\partial^{(i)} v(t)}{\partial t} = \sum_{j=0}^{m} b_j \frac{\partial^{(j)} u(t)}{\partial t}
...
\begin{example}
  Prendendo come esempio il sistema definito in \ref{21-10-D4} abbiamo l'equazione:
  \[
  Mx''(t) + Dx'(t) + Kx(t) = 0
  \] 
  Poniamo:
