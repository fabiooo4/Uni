...
\subsubsection{Modello a cicli di burst CPU-I/O}
Per esaminare gli algoritmi di scheduling si farà riferimento ad un modello astratto del
sistema, cioè il modello a \textbf{cicli di burst CPU-I/O}:
\begin{itemize}
  \item Alternanza CPU e I/O burst (sequenza)
  \item L'esecuzione di un processo consiste nell'alternanza ciclica di un burst di
    CPU e di uno di I/O
\end{itemize}
L'algoritmo di scheduling della CPU agisce soltanto quando ci si trova nelle fasi di
CPU burst, quindi si possono escludere le fasi di I/O burst.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \fill[orange] (1,0) rectangle ++(0.3,-1.5)
      node[midway,left=1cm,black,align=left]
      {load store\\add store\\read from file}
      node[midway,right=0.2cm,black] {CPU burst};

    \fill[green!50!black] (1,-1.5) rectangle ++(0.3,-1.3)
      node[draw,midway,left=1.3cm,black,align=left]
      {wait for I/O}
      node[midway,right=0.2cm,black] {I/O burst};

    \fill[orange] (1,-2.8) rectangle ++(0.3,-0.8)
      node[midway,left=0.7cm,black,align=left]
      {store increment\\index\\write to file}
      node[midway,right=0.2cm,black] {CPU burst};

    \fill[green!50!black] (1,-3.6) rectangle ++(0.3,-1.2)
      node[draw,midway,below left=-0.2cm and 1.3cm,black,align=left]
      {wait for I/O}
      node[midway,right=0.2cm,black] {I/O burst};

    \fill[orange] (1,-4.8) rectangle ++(0.3,-1.8)
      node[midway,left=1cm,black,align=left]
      {load store\\add store\\read from file}
      node[midway,right=0.2cm,black] {CPU burst};

    \fill[green!50!black] (1,-6.6) rectangle ++(0.3,-1.2)
      node[draw,midway,below left=-0.2cm and 1.3cm,black,align=left]
      {wait for I/O}
      node[midway,right=0.2cm,black] {I/O burst};

    \node[scale=1.5] at (-1.2,0.5) {$\vdots$};
    \node[scale=1.5] at (-1.2,-8) {$\vdots$};

    \draw[<-] (-3,-8.5) -- (-3,0.8) node[above] {Time};
  \end{tikzpicture}
  \caption{Modello a cicli di burst CPU-I/O}
\end{figure}

\noindent
Il numero di CPU burst è alto per burst brevi e basso per burst lunghi.

\subsubsection{Preemption (prelazione)}
La \textbf{preemption} è la capacità di interrompere forzatamente un processo in 
esecuzione. In uno scheduling senza preemption, il processo che detiene la CPU non
la rilascia fino al termine del burst. In uno scheduling con preemption, il processo
può essere forzato a rilasciare la CPU prima del termine del burst.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \node[draw] (readyq) at (0,0) {Ready queue};
    \node[draw,right=of readyq,minimum width=0.3cm,minimum height=1cm] (buf1) {};
    \node[draw,circle,right=of buf1] (cpu) {CPU};
    \node[draw,below=of buf1] (ioq) {I/O queues};
    \node[draw,circle,left=of ioq] (io) {I/O};

    \draw[->] (readyq) -- (buf1) -- (cpu);
    \draw[->] (cpu) -- ++(1,0);
    \draw[->] (cpu) |- (ioq) -- (io);
    \draw (io) -| (-1.5,52 |- readyq);
    \draw[<-] (readyq) -- ++(-2,0);
  \end{tikzpicture}
  \caption{Non-Preemptive}
\end{figure}
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \node[draw] (readyq) at (0,0) {Ready queue};
    \node[draw,right=of readyq,minimum width=0.3cm,minimum height=1cm] (buf1) {};
    \node[draw,above=0.5cm of buf1,minimum width=0.3cm,minimum height=1cm] (buf2) {};
    \node[draw,circle,right=of buf1] (cpu) {CPU};
    \node[draw,below=of buf1] (ioq) {I/O queues};
    \node[draw,circle,left=of ioq] (io) {I/O};
    ...
    \draw (cpu) |- (buf2) -| (-1.5,52 |- readyq);
  \end{tikzpicture}
  \caption{Preemptive}
\end{figure}

\noindent Lo schema complessivo degli stati di un processo quindi diventa il seguente:
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=3cm and 2cm,>=stealth',auto, every place/.style={draw}]
    \node [place] (nuovo) [scale=1] {Nuovo};
    ...
    \node [place] (attesa) [below right of=pronto, scale=0.8, align=center,xshift=-0.85cm]
      {In attesa};
    \node [place] (swap) [above right of=pronto, scale=0.8, align=center,xshift=-0.85cm]
      {Swapped};
    \node [place] (exec) [right of=pronto, scale=0.7, align=center] {In\\esecuzione};
    \node [place] (finito) [right of=exec, scale=1, align=center] {Finito};
    ...
    \path[->] (swap) edge node[above left,scale=0.8,align=center]
      {Swap in} (pronto);
    \path[<-] (swap) edge node[above right,scale=0.8,align=center]
      {Swap out} (exec);

  \end{tikzpicture}
  \caption{Stati di un processo (schema complessivo)}
\end{figure}

\noindent
Ci sono più tipi di preemption:
\begin{itemize}
  \item \textbf{Preemption per priorità}: Un processo con priorità più alta può
    interrompere un processo con priorità più bassa
  \item \textbf{Preemption per time-out}: Un processo può essere interrotto
...
\begin{example}
  Gli algoritmi di scheduling a priorità hanno una colonna in più nella tabella
  dei processi, cioè la priorità:
  \begin{table}[H]
    \centering
  ...
  \noindent
  Questa colonna viene calcolata in base all'algoritmo utilizzato. I processi
  verranno messi in esecuzione in base alla priorità (\textbf{se sono arrivati nella coda}),
  se due processi hanno la stessa priorità si una priorità secondaria, ad esempio
  l'ordine di arrivo.
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}
      ...
      \draw[black!50] (0*\xscale,0*\yscale) -- (0*\xscale,-5*\yscale);
      \draw[black!50] (\endtime*\xscale,0*\yscale) -- (\endtime*\xscale,-5*\yscale);

      % Time
      \node[left,scale=0.8] at (0*\xscale,0.5*\yscale) {Tempo};
      \draw[->] (0*\xscale,0.5*\yscale) -- (\endtime*\xscale+1*\xscale,0.5*\yscale);
      \draw[black!50] (0*\xscale,0*\yscale) -- (\endtime*\xscale,0*\yscale);
      \foreach \x in {0,1,...,\endtime} {
        \draw (\x*\xscale,0.5*\yscale) node[above,scale=0.6] {\( \x \)}
          -- ++(0,-0.5*\yscale);
      }

      % P1
      \node[left,scale=0.8] at (0*\xscale,-0.5*\yscale) {\( P_1 \)};
      \draw[black!50] (0*\xscale,-1*\yscale) -- (\endtime*\xscale,-1*\yscale);
      \foreach \x[count=\xi] in {6,...,15} {
        \draw[fill,black!50] (\x*\xscale,-1*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (16*\xscale,0*\yscale) -- (16*\xscale,-5*\yscale);

      % P2
      \node[left,scale=0.8] at (0*\xscale,-1.5*\yscale) {\( P_2 \)};
      \draw[black!50] (0*\xscale,-3*\yscale) -- (\endtime*\xscale,-3*\yscale);
      \foreach \x in {0} {
        \draw[fill,black!50] (\x*\xscale,-2*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (1*\xscale,0*\yscale) -- (1*\xscale,-5*\yscale);

      % P3
      \node[left,scale=0.8] at (0*\xscale,-2.5*\yscale) {\( P_3 \)};
      \draw[black!50] (0*\xscale,-2*\yscale) -- (\endtime*\xscale,-2*\yscale);
      \foreach \x in {16,17} {
        \draw[fill,black!50] (\x*\xscale,-3*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (18*\xscale,0*\yscale) -- (18*\xscale,-5*\yscale);

      % P4
      \node[left,scale=0.8] at (0*\xscale,-3.5*\yscale) {\( P_4 \)};
      \draw[black!50] (0*\xscale,-4*\yscale) -- (\endtime*\xscale,-4*\yscale);
      \foreach \x in {18} {
        \draw[fill,black!50] (\x*\xscale,-4*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }

      % P5
      \node[left,scale=0.8] at (0*\xscale,-4.5*\yscale) {\( P_5 \)};
      \draw[black!50] (0*\xscale,-5*\yscale) -- (\endtime*\xscale,-5*\yscale);
      \foreach \x in {1,...,5} {
        \draw[fill,black!50] (\x*\xscale,-5*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (6*\xscale,0*\yscale) -- (6*\xscale,-5*\yscale);
...
\begin{example}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \draw[black!50] (0*\xscale,0*\yscale) -- (0*\xscale,-3*\yscale);

      % Time
      \node[left,scale=0.8] at (0*\xscale,0.5*\yscale) {Tempo};
      \draw[->] (0*\xscale,0.5*\yscale) -- (31*\xscale,0.5*\yscale);
      \draw[black!50] (0*\xscale,0*\yscale) -- (30*\xscale,0*\yscale);
      \foreach \x in {0,1,...,30} {
        \draw (\x*\xscale,0.5*\yscale) node[above,scale=0.6] {\( \x \)}
          -- ++(0,-0.5*\yscale);
      }
      ...
      \draw[black!50] (0*\xscale,-1*\yscale) -- (30*\xscale,-1*\yscale);
      ...
      \draw[black!50] (27*\xscale,0*\yscale) -- (27*\xscale,-3*\yscale);
      \draw[black!50] (0*\xscale,-2*\yscale) -- (30*\xscale,-2*\yscale);
      ...
      \draw[black!50] (30*\xscale,0*\yscale) -- (30*\xscale,-3*\yscale);
      \draw[black!50] (0*\xscale,-3*\yscale) -- (30*\xscale,-3*\yscale);
    \end{tikzpicture}
    \caption{Esecuzione dei processi con FCFS}
  \end{figure}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \draw[black!50] (3*\xscale,0*\yscale) -- (3*\xscale,-3*\yscale);
      \draw[black!50] (0*\xscale,-2*\yscale) -- (30*\xscale,-2*\yscale);
      ...
      \draw[black!50] (6*\xscale,0*\yscale) -- (6*\xscale,-3*\yscale);
      \draw[black!50] (0*\xscale,-3*\yscale) -- (30*\xscale,-3*\yscale);
    \end{tikzpicture}
...
\begin{example}
  Un'esempio di SJF non preemptive è il seguente:
  \begin{table}[H]
    \centering
    \begin{tabular}{c|c|c}
      Processo & Tempo di arrivo & CPU Burst \\
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \draw[black!50] (0*\xscale,0*\yscale) -- (0*\xscale,-4*\yscale);
      \draw[black!50] (\endtime*\xscale,0*\yscale) -- (\endtime*\xscale,-4*\yscale);
      ...
      \draw[black!50] (7*\xscale,0*\yscale) -- (7*\xscale,-4*\yscale);
      \draw[black!50] (0*\xscale,-1*\yscale) -- (\endtime*\xscale,-1*\yscale);
      ...
      \draw[black!50] (12*\xscale,0*\yscale) -- (12*\xscale,-4*\yscale);
      \draw[black!50] (0*\xscale,-2*\yscale) -- (\endtime*\xscale,-2*\yscale);
      ...
      \draw[black!50] (8*\xscale,0*\yscale) -- (8*\xscale,-4*\yscale);
      \draw[black!50] (0*\xscale,-3*\yscale) -- (\endtime*\xscale,-3*\yscale);
      ...
      \draw[black!50] (16*\xscale,0*\yscale) -- (16*\xscale,-4*\yscale);
      \draw[black!50] (0*\xscale,-4*\yscale) -- (\endtime*\xscale,-4*\yscale);
    \end{tikzpicture}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \draw[black!50] (2*\xscale,0*\yscale) -- (2*\xscale,-4*\yscale);

      \foreach \x[count=\xi] in {11,...,15} {
        \draw[fill,black!50] (\x*\xscale,-1*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      ...
      \draw[black!50] (5*\xscale,0*\yscale) -- (5*\xscale,-4*\yscale);
      ...
      \draw[black!50] (4*\xscale,0*\yscale) -- (4*\xscale,-4*\yscale);
      ...
      \draw[black!50] (11*\xscale,0*\yscale) -- (11*\xscale,-4*\yscale);
    \end{tikzpicture}
    \caption{Esecuzione dei processi con SJF (preemptive)}
  \end{figure}

  \noindent
  All'istante 2 il processo \( P_1 \) viene interrotto perchè arriva il processo
  \( P_2 \) con un burst di CPU più corto. Questo processo viene interrotto a sua
  volta all'istante 4 perchè arriva il processo \( P_3 \) con un burst di CPU ancora
  più corto.
...
\noindent
Siccome il burst di CPU non è noto a priori, si può calcolare una stima del prossimo
burst di CPU in due modi:
\begin{itemize}
  \item Utilizzando le lunghezze dei burst precedenti come proiezione di quelli futuri
  \item Utilizzando la media esponenziale dei burst precedenti:
    \[
      \tau_{n+1} = \alpha \cdot t_n + (1 - \alpha) \cdot \tau_n
    \]
    Dove:
    \begin{itemize}
      \item \( \tau_{n+1} \): Stima del prossimo burst di CPU
      \item \( t_n \): Stima del burst CPU precedente
      \item \( t_n \): Lunghezza reale del burst di CPU appena completato
      \item \( \alpha \): Fattore di previsione (tipicamente \( 0 \leq \alpha \leq 1 \))
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    % Axis
    \draw[->] (0,0) -- (8*\xscale,0) node[right] {Tempo};
    \draw[->] (0,0) -- (0,11*\yscale) node[above] {Burst di CPU};

    % Points t
    \coordinate (t0) at (0*\xscale,3*\yscale);
    \coordinate (t1) at (1*\xscale,1*\yscale);
    \coordinate (t2) at (2*\xscale,3*\yscale);
    \coordinate (t3) at (3*\xscale,1*\yscale);
    \coordinate (t4) at (4*\xscale,10*\yscale);
    \coordinate (t5) at (5*\xscale,10*\yscale);
    \coordinate (t6) at (6*\xscale,10*\yscale);

    % Points tau
    \coordinate (tau0) at (0*\xscale,7*\yscale);
    \coordinate (tau1) at (1*\xscale,5*\yscale);
    \coordinate (tau2) at (2*\xscale,3*\yscale);
    \coordinate (tau3) at (3*\xscale,3*\yscale);
    \coordinate (tau4) at (4*\xscale,2*\yscale);
    \coordinate (tau5) at (5*\xscale,6*\yscale);
    \coordinate (tau6) at (6*\xscale,8*\yscale);
    \coordinate (tau7) at (7*\xscale,9*\yscale);

    % t plot
    \draw[blue,thick] (t0) -- ++(1*\xscale,0)
      -- (t1) -- ++(1*\xscale,0)
      -- (t2) -- ++(1*\xscale,0)
      -- (t3) -- ++(1*\xscale,0)
      -- (t4) -- ++(1*\xscale,0)
    ...
    \draw[red,thick] plot[smooth] coordinates {
        (tau0) (tau1) (tau2) (tau3) (tau4) (tau5) (tau6) (tau7)
      } node[right] {\( \tau_n \)};

    % Ticks
    \foreach \y in {0,2,...,10} {
      \draw (0,\y*\yscale) node[left] {\y} -- ++(0.1,0);
    }

    \foreach \x in {0,1,...,7} {
      \draw (\x*\xscale,0) node[below] {\x} -- ++(0,0.1);
    }
  \end{tikzpicture}
  \caption{Esempio del calcolo del prossimo burst di CPU}
\end{figure}
...
\begin{example}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \node[left,scale=0.8] at (0*\xscale,-0.5*\yscale) {\( P_1 \)};
      \draw[black!50] (0*\xscale,-1*\yscale) -- (\endtime*\xscale,-1*\yscale);
      \foreach \x[count=\xi] in {10,...,19} {
        \draw[fill,black!50] (\x*\xscale,-1*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (20*\xscale,0*\yscale) -- (20*\xscale,-5*\yscale);
      ...
      \draw[black!50] (2*\xscale,0*\yscale) -- (2*\xscale,-5*\yscale);
      ...
      \draw[black!50] (10*\xscale,0*\yscale) -- (10*\xscale,-5*\yscale);
      ...
      \draw[black!50] (8*\xscale,0*\yscale) -- (8*\xscale,-5*\yscale);
      ...
      \draw[black!50] (7*\xscale,0*\yscale) -- (7*\xscale,-5*\yscale);

    \end{tikzpicture}
    \caption{Scheduling a priorità con HRRN (senza preemption)}
  \end{figure}
...
\begin{example}
  Un esempio di Round Robin, con \( q=2 \), è il seguente:
  \begin{table}[H]
    \centering
    \begin{tabular}{c|c|c}
      Processo & Tempo di arrivo & CPU burst \\
  ...
  \begin{table}[H]
    ...
    \begin{tabular}{c|c|c|c|c|c|c|c|c|c}
      ...
      \hline
      In esecuzione & $P_1$ & $P_2$ & $P_3$ & $P_4$ & $P_1$ & $P_3$ & $P_1$ & $P_3$ & $P_3$ \\
      \hline
      Ready queue & $P_2$ & $P_3$ & $P_4$ & $P_1$ & $P_3$ & $P_1$ & $P_3$ & & \\
                  & $P_3$ & $P_4$ & $P_1$ & $P_4$ & & & & \\
                  & $P_4$ & $P_1$ & & & & & & \\
    \end{tabular}
  \end{table}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \foreach \x[count=\xi] in {7,8} {
        \draw[fill,black!50] (\x*\xscale,-1*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (9*\xscale,0*\yscale) -- (9*\xscale,-4*\yscale);

      \foreach \x[count=\xi] in {11} {
        \draw[fill,black!50] (\x*\xscale,-1*\yscale) rectangle ++(1*\xscale,1*\yscale);
      }
      \draw[black!50] (12*\xscale,0*\yscale) -- (12*\xscale,-4*\yscale);
      ...
      \draw[black!50] (3*\xscale,0*\yscale) -- (3*\xscale,-4*\yscale);
      ...
      \draw[black!50] (14*\xscale,0*\yscale) -- (14*\xscale,-4*\yscale);
    ...
    \end{tikzpicture}
    \caption{Scheduling Round Robin con \( q=2 \)}
  \end{figure}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[draw,rectangle,minimum width=5cm,minimum height=0.8cm]
      (1) {Processi si sistema};
    \draw[<-] (1.west) -- ++(-1,0);
    \draw[->] (1.east) -- ++(1,0);

    \node[draw,rectangle,minimum width=5cm,minimum height=0.8cm,below=of 1]
      (2) {Processi interattivi};
    \draw[<-] (2.west) -- ++(-1,0);
    \draw[->] (2.east) -- ++(1,0);

    \node[draw,rectangle,minimum width=5cm,minimum height=0.8cm,below=of 2]
      (3) {Processi interattivi modificabili};
    \draw[<-] (3.west) -- ++(-1,0);
    \draw[->] (3.east) -- ++(1,0);

    \node[draw,rectangle,minimum width=5cm,minimum height=0.8cm,below=of 3]
      (4) {Processi batch};
    \draw[<-] (4.west) -- ++(-1,0);
    \draw[->] (4.east) -- ++(1,0);

    \node[draw,rectangle,minimum width=5cm,minimum height=0.8cm,below=of 4]
      (5) {Processi studente};
    \draw[<-] (5.west) -- ++(-1,0);
    \draw[->] (5.east) -- ++(1,0);

    \node[above] at (1.north) {Priorità più alta};
    \node[below] at (5.south) {Priorità più bassa};
  \end{tikzpicture}
