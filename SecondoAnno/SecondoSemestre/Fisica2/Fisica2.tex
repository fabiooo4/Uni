...
\begin{example}
  ...
  \label{20-05-D19}
  Quindi notiamo che nel solenoide rettilineo indefinito il campo non riesce mai a
  chiudersi, quindi c'è soltanto un campo interno:
  \label{20-05-D20}
  ...
  \[
    \underbrace{\int_{\Gamma_1} B \cdot dl}_{\parallel \text{Est.}} +
    \underbrace{\int_{\Gamma_2} B \cdot dl}_{\perp} +
    \underbrace{\int_{\Gamma_3} B \cdot dl}_{\perp} +
    \underbrace{\int_{\Gamma_4} B \cdot dl}_{\parallel \text{Int.}}
...
\begin{example}
  ...
  \label{20-05-D1}
  Quale forza agisce sulla carica? Il campo è esattamente la linea di campo del filo che passa
  per la carica:
  \label{20-05-D2}
...
\vspace{1em}
\noindent
Questo è il metodo per calcolare l'Ampere, cioè \( 1 N \) di forza che agisce su due
fili paralleli di lunghezza \( 1 m \) distanti \( 1 m \) l'uno dall'altro.
...
\] 
Se si considerano due circuiti \( \Gamma_1 \) e \( \Gamma_2 \) si sa che il campo magnetico
...
Nei circuiti il simbolo dell'induttanza è:
\begin{figure}[H]
  \centering
  \begin{circuitikz}
