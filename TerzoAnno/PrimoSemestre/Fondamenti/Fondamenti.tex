...
\section{Teoria della calcolabilit}
...
\begin{itemize}
  \item Programma, ad esempio Mdt, funzioni parziale ricorsive, programma in un qualsiasi
    linguaggio di programmazione
  \item Input, ad esempio un numero o una stringa, ecc...
\end{itemize}
Un programma può essere rappresentato nello stesso dominio degli input e quindi
è possibile codificare (univicamente) ogni macchina di Turing in un numero naturale.
Questo processo è chiamato \textbf{aritmetizzazione delle macchine di Turing}.
\begin{example}
  Se consideriamo:
  \[
    \sigma = \{0, \$\} \quad Q = \{q_0\} 
...
\[
  Q \to q_0 < q_1 < \ldots < q_n \quad q_i \le q_j \text{ se } i \le j
\] 
\[
  \Sigma \to s_0 < s_1 < \ldots < s_k \quad s_i \le s_j \text{ se } i \le j
\]
Anche le triple possono essere messe in ordine lessicografico:
\[
  q_{i_1} s_{j_1} m_{k_1} \le q_{i_2} s_{j_2} m_{k_2} \text{ se }
  \begin{cases}
    q_{i_1} \le  q_{i_2} \text{ oppure }\\
    q_{i_1} = q_{i_2} \wedge s_{j_1} \le  s_{j_2} \text{ oppure }\\
    q_{i_1} = q_{i_2} \wedge s_{j_1} = s_{j_2} \wedge m_{k_1} \le  m_{k_2} \\
  \end{cases}
\] 

\subsubsection{Fattorizzazione di Gödel}
Esistono diversi algoritmi per aritmetizzare una MdT, uno di questi è la
\textbf{fattorizzazione di Gödel}. L'idea è quella di associare ad ogni macchina di
Turing un numero naturale unico e codificare la macchina di Turing con gli esponenti
dei numeri della fattorizzazione in numeri primi.
\[
  x \in \mathbb{N} \quad fact(x) = p_1^{x_1} \cdot p_2^{x_2} \cdot \ldots \cdot p_n^{x_n}
  \quad p_i \text{ sono primi}
\] 

\begin{itemize}
  \item \textbf{Fattorizzazione di Gödel di una istruzione}:\\
    Associamo ad ogni simbolo \( y \) della macchina di Turing un numero dispari maggiore di
    1: \( a(y) \). Un istruzione della macchina di Turing è una quintupla:
    \[
      E = \left< q, s, q', s', R | L \right>
    \] 
    Calcoliamo il numero associato all'istruzione \( E \):
    \[
      gn(E) = \Pi_{i=1}^{5} p_i^{a(s_i)}
    \] 
    \begin{example}
      Consideriamo la seguente istruzione:
      \[
        E = \left< q_0, \$, q_0, 0, R \right>
      \] 
  ...
  \item \textbf{Fattorizzazione di Gödel di una macchina di Turing}:\\
    Nello stesso modo si può calcolare il numero di Gödel associato ad una macchina di
    Turing \( M \), cioè una sequenza di istruzioni:
    \[
      M = E_1 E_2 \ldots E_n \quad \text{Programma della MdT}
    \] 
    \[
      gn(M) = \Pi_{i=1}^{n} p_i^{gn(E_i)}
    \] 

  \item \textbf{Decodifica}:\\
    Per decodificare un numero naturale \( x \) e ottenere la macchina di Turing
    associata, si esegue la fattorizzazione in numeri primi di \( x \) e si ricavano le
    istruzioni \( E_i \) dai fattori primi:
    \[
      x = n_1 \cdot \ldots n_k
      = \underbrace{\underbrace{m_1^1 \cdot \ldots \cdot m_5^1}_{\text{Tupla 1}} \;\cdot\; m_1^2 \cdot \ldots \cdot m_5^2 
      \;\cdot\; \ldots \;\cdot\; m_1^n \cdot \ldots \cdot m_5^n}_{\text{Tabella di \( M_x \) }}
    \] 
    Indichiamo con \( M_x \) la macchina di Turing il cui programma è codificato in
    \( x \in \mathbb{N} \). \( M_x \) calcola la funcione \( \varphi_x \), che è una
    notazione per indicare la funzione parziale ricorsiva calcolata dal programma
    codificato in \( x \in \mathbb{N} \).
    \[
      \varphi_x(y) = M_x(y) \text{ (Calcolo la MdT \( M_x \) su input \( y \))}
    \] 
    \( \varphi_x \) è la semantica (funzione calcolata) dal programma \( x \).
    \begin{example}
      Se \( x_1 \) è il programma che calcola \( y+y \) e \( x_2 \) fosse il programma
      che calcola \( 2y \) allora sappiamo che i programmi sono diversi:
      \[
        x_1 \neq x_2
      \] 
...
\subsubsection{Macchina universale (interprete)}
La macchina universale è un modello di una macchina programmabile. È un programma che
prende in input due valori, interpreta il primo come un programma e lo applica al secondo.
\begin{definition}
  Dati:
  \[
    \begin{cases}
      x_1 & \text{Codifica \( M_{x_1} \)} \\
      x_2 & \text{Input da manipolare}
    \end{cases}
  \] 
...
M = decodifica(x_1) # Algoritmo descritto di decodifica che da x_1 costruisce il programma corrispondente
  if ok(M) then M(x_2) # ok(M) e' il parser che verifica se M e' ben formata
  else 
    while true 
      x = x # Diverge
\end{lstlisting}
...
\subsection{Problemi insolvibili}
\subsubsection{Problema della terminazione}
Il problema della terminazione (Halting Problem) è il seguente:
Non esiste una funzione totale ricorsiva e quindi calcolabile \( g \) tale che:
\[
  \forall x. g(x) =
  \begin{cases}
    1 & \text{se } \varphi_x(x) \downarrow \text{ termina} \\
    0 & \text{se } \varphi_x(x) \uparrow \text{ diverge}
  \end{cases}
\] 
\( \varphi_x(x) \downarrow \) rappresenta la funzione calcolabile della MdT di codice \( x \) 
e input \( x \) che termina.
La funzione \( g \) decide (cioè risponde con 0 (diverge) o 1 (termina)) se \( \varphi_x \) 
applicata a \( x \) termina.

\vspace{1em}
\noindent
\textbf{Dimostrazione per assurdo}:\\
Supponiamo che esista una tale funzione \( g \) totale ricorsiva, allora esiste
una macchina di Turing di codice \( i_0 \in \mathbb{N} \) che la calcola.
\[
  g = \varphi_{i_0}
\] 
...
if M_i0(x) == 1 then
  ...
  while true do 
    x = x # Diverge
\end{lstlisting}
L'ipotesi è che \( M_{i_0} \) esiste e termina sempre. Si ha un assurdo per costruzione.
Supponiamo che \( \varphi_{i_0}(i_0) \) converga:
\[
  \varphi_{i_1}(i_1) \downarrow
  \iff \underbrace{h(i_1) = 0}_{\text{per \( h = \varphi_{i_1} \) }}
  \iff \underbrace{g(i_1) = 0}_{\text{per def di \( h \) }}
  \iff \underbrace{\varphi_{i_1}(i_1) \uparrow}_{\text{per def di \( g \)}} \quad \text{Assurdo}
\] 
Supponiamo che \( \varphi_{i_0}(i_0) \) diverga:
\[
  \varphi_{i_1}(i_1) \uparrow
  \iff \underbrace{h(i_1) \uparrow}_{\text{per \( h = \varphi_{i_1} \) }}
  \iff \underbrace{g(i_1) = 1}_{\text{per def di \( h \) }}
  \iff \underbrace{\varphi_{i_1}(i_1) \downarrow}_{\text{per def di \( g \)}} \quad \text{Assurdo}
\] 
Si dovrebbero aspettare infiniti passi per decidere se \( \varphi_x(x) \) termina o diverge,
quindi \( g \) non è calcolabile. Non esiste quindi un programma in grado di decidere
\textbf{sempre} se un altro programma termina su un suo input:
\[
  \not \exists \psi . \psi(xy) =
  \begin{cases}
...
\begin{definition}
  ...
  \[
    \forall x . f(x) = 
    \begin{cases}
      1 & \text{se } \varphi_x(x) \text{ è totale} \\
      0 & \text{se } \varphi_x(x) \text{ non è totale}
    \end{cases}
  \] 
  ...
  \] 
\end{definition}
Alcuni problemi \textbf{insolubili} sono:
\begin{itemize}
  \item 
  ...
  \item 
    Decidere se \( \varphi_x \) non restituisce mai un valore fissato \( m \) , cioè se:
    \[
      \forall y . \varphi_x(y) \ne m
    \] 

  \item Decidere se \( \varphi_x = \varphi_y \)
\end{itemize}

\subsection{Specializzatore}
\begin{theorem}[smn (specializzatore)]
  Consideriamo una programma con due input:
  \[
    f(x,y) = \lambda x y . x + y
  \] 
  \textbf{Specializzare} equivale a costruire un nuovo programma in cui una parte
  degli input è integrata nel codice.
  Usiamo \( f \) per ottenere \( f' \) che somma ad un valore in unput il valore \( 5 \).
  \[
    f'(y) = \lambda y . 5 + y
  \] 
  Si è quindi speciallizzata \( f \) a 5 per \( x \). Se si specializza su tutti gli
  input, il programma diventa costante. La specializzazione su input parziali viene
  detta \textbf{partial computation}.

  \vspace{1em}
  \noindent
  Quindi in modo formale:
  \( \forall n, m \ge 1 \) esiste una funzione \textbf{totale} ricorsiva \( s \) tale che
  \[
    \begin{aligned}
      &\forall x, y_1, \ldots, y_m \;.\; \lambda z_1 \ldots z_n . \\
      &\underbrace{\varphi_x}_{\text{Codice}}(\overbrace{y_1 \ldots y_m}^{\text{Input specializzati}}
      z_1 \ldots z_n)\\
      &= \lambda z_1 \ldots z_n . \varphi_s(x, y_1 \ldots y_m)
    \end{aligned}
  \]
  Dove \( \varphi_s(x, y_1 \ldots y_m) \) è la trasformazione totale \( S \) del codice \( x \) 
  e considerando gli input specializzati \( y_1 \ldots y_m \) 

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione}: Consideriamo \( m = n = 1 \) (caso particolare, ma \( m > 1 \) e
  \( n > 1 \) è analogo) con:
  \[
  ...
  \] 
  dove \( \varphi_{x_0} \) denota la funzione calcolata dalla macchina di Turing il cui
  codice è aritmetizzato in \( x_0 \). Questa funzione è \textbf{intuitivamente calcolabile},
  di conseguenza esiste una macchina di Turing che la calcola (tesi di Church) il cui
  codice dipende (ed è costruibile ricorsivamente) da \( x_0 \) (codice originale) e
  \( y_0 \) (l'input specializzato). Quindi esiste una trasformazione \textbf{totale ricorsiva}
  tale che il codice è \( s(x_0, y_0) \):
  \[
    \varphi_{s(x_0, y_0)}(z) = \lambda z . \lambda z . \varphi_{x_0}(y_0, z)
  \] 
\end{theorem}

\subsection{Prima proiezione di Futamura}
La proiezione di Futamura è una tecnica per ottenere un compilatore a partire
da un interprete e uno specializzatore.

\begin{definition}
  Dato un programma sorgente \( source \in L \) esiste una trasformazione di
  \( source \) in un programma (semanticamente equivalente) \( target \in L' \).

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione}: Supponiamo che \( int \) sia la macchina universale:
  \[
    int(p, d) = p(d)
  \] 
  dove:
  \begin{itemize}
    \item \( p \) è un programma
    \item \( d \) è un dato di input
  \end{itemize}
  Supponiamo che \( spec \) sia uno specializzatore:
  \[
    spec(Q, d) = Q'_d \text{ integra nel codice l'input specializzato } d
  \] 
  dove:
  \begin{itemize}
    \item \( Q \) è un programma a due input
  \end{itemize}
  Allora:
  \[
...
\subsubsection{Linguaggio Turing completo}
Un linguaggio Turing completo permette di calcolare tutte le funzioni intuitivamente calcolabili.
Un linguaggio Turing completo equivale ad una macchina di Turing. I costrutti 
minimi per avere un linguaggio Turing completo sono:
\[
  \begin{aligned}
    &\exists p \;.\; \varphi_p = \lambda x . 0\\
    &\exists p \;.\; \varphi_p = \lambda x . x + 1\\
    &\exists p \;.\; \varphi_p = \lambda x . x \text{ (proiezione)}\\
    &\exists p \;.\; \varphi_p = \varphi_Q \circ \varphi_H \text{ (composizione)}\\
    &\exists p \;.\; \varphi_p = \varphi_{\text{While}} B \; \{Q\} \text{ (minimizzazione)}
    &\exists p \;.\; \varphi_p(x,y) = \varphi_x(y)\\
    &\exists p \;.\; \varphi_{spec(x,y)}(z) = \lambda z . \varphi_x(y,z) \text{ (proiezione)}
  \end{aligned}
\] 
Se consideriamo la \textbf{primitiva ricorsione} abbiamo solo cicli for. Un linguaggio che non
permette cicli non determinati (while) \textbf{non} è Turing completo.

\vspace{1em}
\noindent
I linguaggi di Turing sono tutti quei linguaggi accettati da una mcchina di Turing:
\[
  \exists  MdT \; m \;.\; L = L(M) = \left\{ \left. y \;\right|\; \varphi_m(y) \downarrow \right\}
\] 
dove \( y \) sono le stringhe su cui la MdT termina. E \( \varphi_x \) è la funzione
parziale ricorsiva calcolata da una MdT di codice \( x \).

\vspace{1em}
\noindent
\begin{definition}
  Il dominio di una funzione parziale ricorsiva \( \varphi_x \) è l'insieme:
  \[
    dom(\varphi_x) = \left\{ \left. y \;\right|\; \varphi_x(y) \downarrow \right\}
  \] 
\end{definition}


\section{Teoria della ricorsione}
La teoria della ricorsione parla degli insiemi (linguaggi) caratterizzati dalle macchine
di Turing.

\subsection{Insiemi ricorsivamente enumerabili}
\begin{definition}
  Un insieme \( I \subseteq \mathbb{N} \) è ricorsivamente enumerabile (RE) se è
  il dominio di una funzione parziale ricorsiva, cioè:
  \[
    \exists \varphi_x \text{ parziale ricorsiva} \;.\; dom(\varphi_x) = I
  \] 
  Cioè se esiste una macchina di Turing che si ferma su \textbf{tutti} gli elementi di \( I \).

  \vspace{1em}
  \noindent
  \textbf{Notazione}: Per rappresentare un insieme RE si usa la notazione:
  \[
    W_x \stackrel{def}{=} dom(\varphi_x)
  \] 
  Se abbiamo \( \varphi_x \) parziale ricorsiva, allora \( W_x \) è RE:
  \[
    \varphi_x \text{ PR} \implies W_x = dom(\varphi_x) \text{ è RE}
  \] 
  Se abbiamo \( I \subseteq \mathbb{N} \) e riusciamo a costruire la sua \textbf{funzione
  semicaratteristica}, cioè: 
  \[
    \psi_I(x) =
    \begin{cases}
      1 & \text{se } x \in I \\
  ...
  \] 
\end{definition}
Per dimostrare che un insieme è RE bisogna fornire l'algoritmo della funzione semicaratteristica
che deve terminare se e solo se ha in input un elemento dell'insieme.

\begin{example}
  Consideriamo una variante del problema della terminazione, cioè quello che determina i
  programmi che terminano su \textbf{almeno un input}.
  Si può riscrivere con il seguente insieme:
  \[
    \left\{ \left. x \;\right|\; \exists y \;.\; \varphi_x(y) \downarrow \right\}
  \] 
  Consideriamo il caso specifico applicato all'input 0:
  \[
    I = \left\{ \left. x \;\right|\; \varphi_x(0) \downarrow \right\}
  \] 
  L'algoritmo della funzione semicaratteristica è:
\begin{lstlisting}[language=Python]
input(x)
stop = 0
build phi_x # Esiste algoritmo ricorsivo che costruisce la MdT di codice x
while !stop do {
  execute next step of phi_x(0)
  if phi_x(0) halts then {
    output(1)
    stop = 1
  }
}
output(1)
\end{lstlisting}
  La funzione è semicaratteristico perchè se \( \varphi_x(0)\downarrow \) restituisce
  1, altrimenti diverge.
\end{example}

\begin{example}
  Considderiamo l'insieme del problema della terminazione generale:
  \[
    I = \left\{ \left. x \;\right|\; \exists y \;.\; \varphi_x(y)\downarrow \right\}
  \] 
...
stop = 0
while !stop do {
  execute next step of phi_x(y)
  if phi_x(y) halts then {
    stop = 1
  }
}
return(1)
\end{lstlisting}
  Questo programma è sbagliato perchè se diverge su 0, non potrà mai testare \( y > 0 \),
  ma \( \varphi_x(1) \) potrebbe terminare. Quindi questo algoritmo diverge anche per
  \( x \in I \) e di conseguenza non è la funzione semicaratteristica di \( I \).

  \vspace{1em}
  \noindent
  Un modo per risolvere questo problema è eseguire in parallelo tutte le \( \varphi_x(y) \)
  per \( y = 0, 1, 2, \ldots \). Siccome non si può eseguire in parallelo, si può simulare
  l'esecuzione in parallelo eseguendo un passo di ogni \( \varphi_x(y) \) in modo ciclico
  facendo partire una nuova computazione ad ogni ciclo. In questo modo se c'è almeno un
  input su cui prima o poi la macchina termina, prima o poi lo trova:
  \begin{itemize}
    \item \textbf{Passo 0:} Esegue un passo di \( \varphi_x(0) \).
      \begin{itemize}
        \item Se termina (\( \downarrow \)) allora finisce
        \item Se non termina (\( \uparrow \)) allora si esegue il passo successivo
      \end{itemize}

    \item \textbf{Passo n+1:} Esegue un passo per \( \varphi_x(0), \varphi_x(1), \ldots, \varphi_x(n) \) 
      (se non sono terminate prima). Esegue il primo passo di \( \varphi_x(n+1) \).
  \end{itemize}
  Questo algoritmo si chiama \textbf{Dovetail}.
...
\begin{lstlisting}[language=Python]
input(x)
build phi_x # Esiste una procedura ricorsiva per costruire phi_x da x
...
while !stop do {
  for i from 0 to y {
    execute next step of phi_x(i)
    if phi_x(i) halts then {
      stop = 1
    }
    y++
  }
...
Esiste un algoritmo che decide se un elemento sta o meno nell'insieme. L'insieme ricorsivo
è caratterizzato da una funzione totale (\( \implies \) dominio in \( \mathbb{N} \)).
\begin{definition}
  Un insieme \( I \) è ricorsivo se esiste una funzione totale ricorsiva \( f_I \) tale che:
  \[
    f_I(x) =
    \begin{cases}
      1 & \text{se } x \in I \\
    ...
    \end{cases}
  \] 
  Questa funzione è detta \textbf{funzione caratteristica} di \( I \).
\end{definition}
Per dimostrare che un insieme è ricorsivo bisogna fornire l'algoritmo che calcola la
sua funzione caratteristica.
\begin{example}
  Un esempio di insieme ricorsivo è l'insieme dei numeri pari:
  \[
    I = \left\{ \left. x \in \mathbb{N} \;\right|\; x \text{ è pari} \right\}
  \] 
  L'algoritmo che calcola la funzione caratteristica \( f \) è:
\begin{lstlisting}[language=Python]
input(x)
y = x mod 2 # Resto della divisione per 2 (funzione PR)
z = neg(y) # y = 0 -> z = 1; y = 1 -> z = 0
output(z)
\end{lstlisting}
...
\[
  f(x) = 
  \begin{cases}
    1 & \text{se } x \in I \text{ (x è pari)} \\
    0 & \text{se } x \notin I \text{ (x è dispari)}
  \end{cases}
\] 
\end{example}
...
\begin{example}
  ...
  \[
    I = \left\{ \left. x \;\right|\; \exists n \;.\; x = n^2 \right\}
  \] 
...
output(1)
\end{lstlisting}
  Questo programma però non è la funzione caratteristica di \( I \) perchè non può essere
  totale. Bisogna quindi trovare un modo per far terminare il programma anche se \( x \notin I \).
\begin{lstlisting}[language=Python]
input(x)
n = 0
while n <= x do {
...
\begin{theorem}
  Se l'insieme \( A \) è ricorsivo, allora è anche ricorsivamente enumerabile.
  \[
    \exists f_A(x) =
    \begin{cases}
      1 & \text{se } x \in A \\
  ...
  \[
    ...
    \begin{cases}
      1 & \text{se } f_A(x) = 1 \\
      \text{while true}\;(\uparrow) & \text{se } f_A(x) = 0 
    \end{cases}
  \] 
  Supponiamo \( C_A \) è l'algoritmo che codifica \( f_A \):
\begin{lstlisting}[language=Python]
input(x)
y = C_A(x) # Esiste per ipotesi ed e' totale
  if y = 1 then output(1)
  else while true do x = x # Diverge
\end{lstlisting}
...
\begin{theorem}[Teorema di Post]
  Un insieme \( A \) è ricorsivo se e solo se \( A \) e \( \bar{A} \) sono entrambi 
  ricorsivamente enumerabili.

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione:} Se \( A \) è ricorsivo, allora \( A \) è RE (teorema precedente) e
  analogamente costruiamo la sue semicaratteristica per \( \bar{A} \):
  \[
    f_{\bar{A}}(x) =
    \begin{cases}
      1 & \text{se } f_A(x) = 0 \\
...
stop = 0
while !stop do {
  execute next step of C_A(x)
  if C_A(x) halts then output(1)
  execute next step of C_barA(x)
  if C_barA(x) halts then output(0)
}
\end{lstlisting}
\end{theorem}
...
\begin{example}
  ...
  \[
    A_m = \left\{ \left. x \;\right|\; \varphi_x(x) \downarrow \text{ in meno di \( m \) passi} \right\}
  \] 
  Questo insieme è ricorsivo perchè \( m \) è un input che limita il numero di passi della
  computazione. Per dimostrarlo forniamo l'algoritmo della funzione caratteristica:
  \[
    f_{A_m}(x) =
    \begin{cases}
      1 & \text{se } x \in A_m \\
...
\begin{lstlisting}[language=Python]
input(x, m)
build phi_x # Esiste procedura ricorsiva per costruire la MdT di codice x
for i from 1 to m do {
  execute next step of phi_x(x)
  if phi_x(x) halts then { # In questo caso la phi_x(x) ha terminato in meno di m passi
    output(1)
  }
}
output(0) # phi_x(x) e' arrivata a m passi senza terminare
\end{lstlisting}
\end{example}

\subsection{Insieme K}
Vogliamo mostrare che l'insieme dei ricorsivi è un sottoinsieme stretto dei RE:
\[
  R \subset RE
\] 
Per farlo introduciamo l'insieme \( K \), cioè l'insieme ricorsivamente
enumerabile e \textbf{non} ricorsivo:
\[
  K \stackrel{def}{=} \left\{ \left. x \;\right|\; \varphi_x(x) \downarrow \right\}
\] 

\subsubsection{Dimostrazione}
Per dimostrare che \( K \) è RE dobbiamo fornire l'algoritmo della semicaratteristica:
\[
  \psi_K(x) =
  \begin{cases}
    1 & \text{se } x \in K \\
...
while !stop do {
  execute next step of phi_x(x)
  # Se esiste un numero di passi tale che phi_x(x) termina, allora il ciclo termina altrimenti diverge
  if phi_x(x) halts then {
    stop = 1
  }
}
output(1)
\end{lstlisting}
Abbiamo dimostrato che \( K \) è RE.
\begin{define}
  Sappiamo quando una funzione parziale ricorsiva termina: 
  \[
    \varphi_x(y) \downarrow \iff \exists n \in \mathbb{N} \;.\; \varphi_x(y)
    \text{ termina dopo esattamente } n \text{ passi}
  \] 
  e quando diverge:
  \[
    \varphi_x(y) \uparrow \iff \forall n \in \mathbb{N} \;.\; \varphi_x(y)
    \text{ non termina dopo aver eseguito } n \text{ passi}
  \] 
\end{define}
Ora dimostriamo che \( K \) non è ricorsivo per assurdo. Supponiamo che \( K \) sia
ricorsivo, quindi per il teorema di Post anche \( \bar{K} \) è RE. Se \( \bar{K} \) è RE,
allora, per definizione, è il dominio di una funzione parziale ricorsiva:
\[
  \exists y_0 \;.\; \bar{K} = dom(\varphi_{y_0}) \stackrel{def}{=} W_{y_0}
\] 
quindi:
\[
  \begin{aligned}
    y_0 \in \bar{K} = W_{y_0} &\stackrel{\text{def di \( W_{y_0} \)}}{\iff}
    y_0 \in dom(\varphi_{y_0}) \\
                              &\stackrel{\text{def di dom}}{\iff} \varphi_{y_0}(y_0) \downarrow \\
                              &\stackrel{\text{def di \( K \)}}{\iff} y_0 \in K \quad \text{Assurdo}
  \end{aligned}
\]
Quindi \( K \) non è ricorsivo. Il suo complemento è definito come:
\[
  \begin{aligned}
    \bar{K} &= \left\{ \left. x \;\right|\; \neg \varphi_x(x) \downarrow \right\}\\
            &= \left\{ \left. x \;\right|\; \varphi_x(x) \uparrow \right\}
  \end{aligned}
\] 

\subsection{Caratterizzare RE e RIC}
\subsubsection{Teorema di caratterizzazione degli insiemi RE}
\begin{theorem}
  Le seguenti affermazioni sono equivalenti:
  \begin{enumerate}
    \item \( A \subseteq \mathbb{N} \) è RE, cioè
      \( \exists \psi \in PR \;.\; A = dom\; \psi \) 

    \item \( \exists \varphi \in PR \;.\; A = range\; \varphi \), dove
      \[
        range\; \varphi = \left\{ \left. \varphi(x) \;\right|\; x \in dom\; \varphi \right\}
      \]
      cioè l'insieme degli elementi restituiti da \( \varphi \) per tutti gli input su cui
      termina.

    \item \( A = \varnothing \) oppure \( \exists f \text{ totale ricorsiva} \;.\; A = range\; f \).
      Essendo \( f \) totale, termina su tutti gli input, quindi:
      \[
        range\; f = \left\{ \left. f(x) \;\right|\; x \in \mathbb{N} \right\}
      \] 
  \end{enumerate}
\end{theorem}

\subsubsection{Teorema di caratterizzazione degli insiemi ricorsivi}
\begin{theorem}
  Le seguenti affermazioni sono equivalenti:
  \begin{enumerate}
    \item \( A \) è ricorsivo

    \item \( A = \varnothing \) oppure \( \exists f \) totale ricorsiva \textbf{non decrescente}
      tale che \( A = range\; f \). Una funzione non decrescente se:
      \[
        \forall x, y \;.\; x \le y \implies f(x) \le f(y)
      \] 
  \end{enumerate}
\end{theorem}

\subsubsection{Teorema di ricorsione di Kleene}
\begin{theorem}
  \label{thm:kleene}
  Sia \( t \) una funzione totale ricorsiva, allora esiste un codice \( n \in \mathbb{N} \) tale
  che:
  \[
    \varphi_n = \varphi_{t(n)}
  \] 
  Cioè \( t(n) \) trasforma il codice \( n \) ma la semantica \( \varphi  \) dei due codici
  è la stessa. Di conseguenza per ogni trasformazione esiste una programma tale che la
  trasformazione modifica il codice ma non la funzione calcolata (semantica)

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione}: Consideriamo il codice \( u \in \mathbb{N} \) di una macchina
  di Turing \( \varphi_u \). Definiamo:
  \[
    \psi(u, x) = 
    \begin{cases}
    ...
    \end{cases}
  \] 
  Dobbiamo verificare se \( \psi  \) è parziale ricorsiva, quidi dobbiamo fornire
  l'algoritmo che la calcola e che deve terminare quando la funzione deve terminare:
\begin{lstlisting}[language=Python]
  input(u, x)
  build phi_u # Esiste procedura ricorsiva per costruire la MdT di codice u
  y = phi_u(u) # Se phi_u(u) non termina, il programma non termina, se termina, il risultato viene messo in y
  build phi_y # Esiste procedura ricorsiva per costruire la MdT di codice y
  output(phi_y(x)) # Esegue phi_y su x, se non termina, il programma non termina, se termina, restituisce il risultato
\end{lstlisting}
  Quindi \( \psi(u, x) \) è parziale ricorsiva.

  \vspace{1em}
  \noindent
  Poichè \( \psi(u, x) \) è parziale ricorsiva, allora si può applicare il teorema smn:
  \[
    \exists g \text{ totale ricorsiva} \;.\; \psi(u, x) = \varphi_{g(u)}(x)
  \] 
  Prendiamo \( t \) totale ricorsiva e consideriamo:
  \[
    t \circ g \text{ composizione totale ricorsiva}
  \] 
  Essendo \( t \circ g \) ricorsiva, allora esiste una macchina di Turing che la calcola
  con \( v \) l'indice di tale macchina, ovvero \( \varphi_v \) è la funzione calcolata:
  \[
    t \circ g = \varphi_v
  \] 
  Quindi \( t \circ g \) è la funzione calcolata dalla macchina di Turing di indice \( v \).

  \vspace{1em}
  \noindent
  Calcoliamo ora \( \psi(v, x) \):
  \[
    \begin{aligned}
      \psi(v, x) &\stackrel{smn}{=} \varphi_{g(v)}(x)\\
                 &\stackrel{\text{def di } \psi}{=}
                 \begin{cases}
                    \varphi_{\varphi_v(v)}(x) & \text{se } \varphi_v(v) \downarrow \\
                    \uparrow & \text{altrimenti}
                 \end{cases}\\
                 &\stackrel{\varphi_v = t \circ g}{=}
                 \begin{cases}
                   \varphi_{t \circ g(v)}(x) & \text{se } \varphi_v(v) = t \circ g(v) \downarrow \\
                    \uparrow & \text{altrimenti}
                 \end{cases}
    \end{aligned}
  \] 
  Ma \( t \circ g \) è totale e termina sempre, quindi la condizione è sempre vera:
  \[
    \psi(v, x) = \varphi_{g(v)}(x) = \varphi_{t \circ g(v)}(x)
  \] 
  Quindi se prendiamo:
  \[
    n = g(v) \implies \varphi_n(x) = \varphi_{t(n)}(x) \; \forall x \in \mathbb{N}
  \] 
\end{theorem}

\subsection{Dimostrare che un insieme è ricorsivo}
Per dimostrare che un insieme \( A \) è ricorsivo, si usano i seguenti metodi:
\begin{itemize}
  \item Se \( A \) è un \textbf{Insieme generico}: Per dimostrare che \( A \) è ricorsivo forniamo l'algoritmo per
    la funzione caratteristica \( f \) 

  \item Se \( A \) è definita tramite una funzione:
    \[
      A = \left\{\left. f(x) \;\right|\; x \in \mathbb{N} \right\}
    \] e rappresenta esattamente il \( range\;f \) si dimostra che \( f \) è totale monotona non decrescente.
    \begin{itemize}
      \item Se è finito allora \( A \) è ricorsivo
      \item Se è infinito si dimostra che \( f \) non decresce per dimostrare che \( A \) è ricorsivo
    \end{itemize}
\end{itemize}
...
\begin{example}
  ...
  \[
    ...
    \end{cases}
  \] 
  Studiare l'insieme \( A = range\; f \).

  \vspace{1em}
  \noindent
  La funzione \( f \) è totale in quanto definita ricorsivamente (primitiva ricorsione).
  Controlliamo com'è fatta la funzione:
  \[
  \begin{aligned}
    f(0) &= 2\\
    f(1) &= 2 \cdot f(0) = = 4\\
  ...
  \end{aligned}
  \] 
  Notiamo che questa funzione rappresenta le potenze di 2, quindi il range è:
  \[
    range\; f = \left\{ \left. 2^n \;\right|\; n \ge 1 \right\}
  \] 
  Si possono usare due metodi per dimostrare che \( A \) è ricorsivo:
  \begin{enumerate}
    \item Primo metodo
      \begin{enumerate}
        \item Dimostrare per induzione che:
          \[
            range\; f = \left\{ \left. 2^n \;\right|\; n \ge 1 \right\}
            \] 

          \item Fornire l'algoritmo per la funzione caratteristica
        \end{enumerate}

      \item Secondo metodo
        \begin{enumerate}
          \item 
            Dimostrare per induzione che \( f \) è non decrescente
        \end{enumerate}
  \end{enumerate}

  \vspace{1em}
  \noindent
  Svolgiamo il secondo metodo, cioè dimostriamo che \( f \) è non decrescente:
  \[
    \underbrace{\forall  n > 0 \;.\; f(n) \ge f(n-1)}_{\text{Tesi}} \equiv f(n) \le f(n+1)
  \] 
  \begin{itemize}
    \item \textbf{Caso base}:\\
      \( n = 1 \) dobbiamo verificare \( f(1) \ge f(0) \):
      \[
        f(1) = 4 \ge 2 = f(0) \quad \checkmark
      \] 

    \item \textbf{Passo induttivo}:\\
      Prendiamo \( n >1 \). L'ipotesi induttiva è:
      \[
        \forall h \le n \;.\; f(h) \ge f(h-1)
      \] 
      Dimostriamo che vale per \( n \):
      \[
        f(n) = 2 f(n-1) \ge f(n-1) \quad \checkmark
      \] 
  \end{itemize}
  Abbiamo dimostrato che \( f \) è non decrescente, quindi \( A \) è ricorsivo.
\end{example}

\subsection{Dimostrare che un insieme non è ricorsivo}
Per dimostrare che un insieme non è ricorsivo si usa il \textbf{teorema di Rice}
L'insieme \( A \) non è ricorsivo se:
\begin{itemize}
  \item Se l'insieme \( A \) è estensionale e non banale (teorema di Rice)
  \item Se l'insieme \( A \) non è estensionale va costruito un assurdo
\end{itemize}

\subsubsection{Proprietà}
\begin{definition}[Proprietà su \( \mathbb{N} \)]
  Un qualunque sottoinsieme di un "universo" rappresenta una proprietà sugli
  elementi di tale universo.

  \vspace{1em}
  \noindent
  Grazie all'aritmetizzazione delle macchine di Turing si ha che una proprietà
  sugli elementi di \( \mathbb{N} \) corrisponde ad una proprietà sui programmi:
  \[
    x \subseteq \mathbb{N} \implies \text{proprietà sui programmi (MdT)}
  \] 
  Dove:
  \begin{itemize}
    \item \( N \equiv \) essere un programma
    \item \( \varnothing \equiv \) non essere un programma
  \end{itemize}
  La quantità di proprietà sui programmi è \( \mathcal{P}(\mathbb{N}) \) quindi non
  numerabile.
\end{definition}

\begin{example}
  Per rappresentare la proprietà di essere pari:
  \[
    x \text{ è pari se } x \;div\; 2 = \text{ resto } 0
  \] 
  Quindi:
  \[
    \left\{ \left. x \in \mathbb{N} \;\right|\; x \mod 2 = 0 \right\}
  \] 
  è l'insieme di tutti e i soli numeri pari. Rappresenta in modo
  estensionale la proprietà dei numeri pari.
\end{example}

\begin{example}
  I programmi che hanno 50 righe di codice sono la seguente proprietà:
  \[
    \left\{\left. P \in MdT \;\right|\; P \text{ ha 50 righe di codice} \right\}
  \] 
  I programmi che restituiscono la costante 5 sono la seguente proprietà:
  \[
    \left\{\left. P \in MdT \;\right|\; \forall x \;.\; P(x) = 5 \right\}
  \] 
\end{example}


\subsubsection{Proprietà estensionale}
\begin{definition}[Proprietà estensionale]
  La proprietà \( \Pi \in \mathbb{N} \) è estensionale se:
  \[
    \forall x, y \in \mathbb{N} \;.\; x \in \Pi \wedge \varphi_x = \varphi_y \implies y \in \Pi
  \] 
  Cioè se il programma \( x \) soddisfa la proprietà \textbf{e} i programmi \( x \) e \( y \) 
  calcolano la stessa funzione, allora anche \( y \) soddisfa la proprietà.

  \vspace{1em}
  \noindent
  Un insieme è estensionale ogni volta che nella condizione compare solo la funzione
  calcolata e non il codice del programma.
\end{definition}
\begin{example}
  \( P \in \Pi_1 \) allora \( P \) ha 50 righe di codice.\\
  \( P' = P;\; skip \) allora \( P' \) ha 51 righe di codice.

  \vspace{1em}
  \noindent
  \( P \) e \( P' \) calcolano la stessa funzione, ma:
  \[
    P \in \Pi_1 \wedge P' \notin \Pi_1
  \] 
  Quindi \( \Pi_1 \) \textbf{non è estensionale}
\end{example}
\begin{example}
  Gli insiemi \( \varnothing, \mathbb{N} \) sono estensionali (banali)
\end{example}
\begin{example}
  Verifichiamo che l'insieme \( \left\{ \left. x \;\right|\; \forall y \;.\; \varphi_x(y) = y \right\} \)
    è estensionale.
    \begin{itemize}
      \item Prendiamo \( z \in \Pi  \) per definizione di \( \Pi  \):
        \[
          \varphi_z(y) = y \quad \forall y
        \] 
      \item Prendiamo \( w \) tale che:
        \[
          \varphi_z = \varphi_w \implies \forall y \;.\; \varphi_w(y) = \varphi_z(y) = y
        \] 
        Per definizione di \( \Pi  \) \( w \in \Pi \) 
    \end{itemize}
\end{example}
\begin{example}
  Verifichiamo che l'insieme \( \left\{ \left. x \;\right|\; \varphi_x
    \text{ definitivamente }\footnote{
      Cioè \( \exists m \;.\; \forall n \ge m \;.\; \varphi_x(n) \ge 3 \)} \ge 3 \right\} \) è estensionale.
      \begin{itemize}
        \item Prendiamo \( z \in \Pi \) per definizione di \( \Pi \):
          \[
            \varphi_z \text{ definitivamente } \ge 3 \equiv \exists m \;.\; \forall n \ge m \;.\; \varphi_z(n) \ge 3
          \] 

        \item Prendiamo \( w \) tale che:
          \[
            \begin{aligned}
              \varphi_z = \varphi_w &\implies \forall n \;.\; \varphi_w(n) = \varphi_z(n)\\
                                    &\implies \forall n \ge m \;.\; \varphi_w(n) = \varphi_z(n) \ge 3
            \end{aligned}
          \] 
          Per definizione di \( \Pi \) \( w \in \Pi \)
      \end{itemize}
...
\begin{theorem}[Teorema di Rice]
  Sia \( \Pi  \) una proprietà estensionale (non conta il programma, ma solo il comportamento)
  , allora \( \Pi  \) è ricorsiva se e solo se è banale.

  Cioè le uniche proprietà che descrivono il comportamento di un programma sono 
  quelle banali (vuote o totali).

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione:} Vogliamo dimostrare che:
  \begin{itemize}
    \item \( \Leftarrow \):\\
      Se \( \Pi  \) una proprietà banale, allora
      \[
        \Pi = \varnothing \quad \text{o} \quad \Pi  = \mathbb{N}
      \] 
      e sia \( \varnothing \) che \( \mathbb{N} \) sono ricorsivi.

    \item \( \Rightarrow \):\\
      Se \( \Pi  \) è ricorsiva, allora \( \Pi \) è banale. Ciò equivale a dire
      che se \( \Pi  \) è non banale, allora non è ricorsiva.
  \end{itemize}
  Prendiamo \( \Pi  \) \textbf{ricorsiva estensionale}:
  \[
    \exists g \text{ totale ricorsiva (caratteristica)} \;.\; g(x) =
    \begin{cases}
      1 & \text{se } x \in \Pi \\
      0 & \text{se } x \notin \Pi
    \end{cases}
  \] 
  Supponiamo che \( \Pi  \) non sia banale, allora sappiamo che:
  \begin{itemize}
    \item 
      \(
        \Pi \neq \varnothing \equiv \exists a \in \Pi
      \) 

    \item \( \Pi \neq \mathbb{N} \equiv \exists b \notin \Pi  \) 
  \end{itemize}
  Costruiamo \( h: \mathbb{N} \to \mathbb{N} \) 
  \[
    h(x) =
    \begin{cases}
      a & \text{se } g(x) = 0 \quad (x \notin \Pi) \\
      b & \text{se } g(x) = 1 \quad (x \in \Pi)
    \end{cases}
  \] 
  Per il teorema di ricorsione di Kleene (teorema \ref{thm:kleene}) \( h \) è totale
  ricorsiva perchè lo è anche \( g \):
  \[
    h \text{ totale ricorsiva} \implies \exists n_0 \;.\; \varphi_{n_0} = \varphi_{h(n_0)}
  \] 
  Ci domandiamo dove sia \( n_0 \) rispetto a \( \Pi \):
  \begin{itemize}
    \item 
      Se \( n_0 \in \Pi \quad (\varphi_{n_0} = \varphi_{h(n_0)}) \) allora \( \Pi  \) è
      estensionale, quindi \( h(n_0) \in \Pi  \), ma:
      \[
        h(n_0) = b \notin \Pi \quad \text{ perchè } n_0 \in \Pi 
      \] 

    \item Se \( n_0 \notin \Pi \) allora \( h(n_0) \notin \Pi \) sempre per estensibilità,
      ma:
      \[
        h(n_0) = a \in \Pi \quad \text{ perchè } n_0 \notin \Pi
      \] 
  \end{itemize}
  Quindi \( \Pi  \) deve essere per forza \( \varnothing \) o \( \mathbb{N} \) 
  cioè banale.
\end{theorem}

\begin{example}
  Studiamo se le seguenti proprietà sono estensionali o no:
  \begin{itemize}
    \item \( \left\{ \left. x \;\right|\; \forall y \;.\; \varphi_x(y) \downarrow \right\} \)
        È estensionale perchè dipende solo dalla funzione calcolata.
    \item \( \left\{ \left. P \;\right|\; P \text{ contiene almeno un ciclo while} \right\} \)
        Non è estensionale perchè dipende dal codice del programma.
    \item \( \left\{ \left. x \;\right|\; x \text{ ha dimensione } \ge 1000 \right\} \)
        Non è estensionale perchè dipende dal codice del programma.
    \item \( \left\{ \left. P \;\right|\; P \text{ soddisfa confidenzialità} \right\} \)
        È estensionale perchè dipende solo dalla funzione calcolata.
    \item \( \left\{ \left. x \;\right|\; \varphi_x(x)\downarrow \right\} = K \;(\text{non ricorsivo})\)
        Non è estensionale perchè l'indice è usato come input della funzione.
        ...
        \begin{itemize}
          \item 
            Prendiamo \( z \in \Pi \implies \varphi_z(z) \downarrow\) per
            definizione di \( \Pi  \).

          \item Prendiamo \( w \;.\; \varphi_w = \varphi_z \implies \varphi_w(z) = \varphi_z(z) \downarrow \) 
            per poter dire che \( w \in \Pi  \) dovremmo avere \( \varphi_w(w) \downarrow \) che non
            necessariamente vale e quindi \( \Pi  \) non è estensionale.
        \end{itemize}

      \item \( \Pi_n = \left\{ \left. x \;\right|\; \varphi_x(n) \downarrow \right\} \)
          È estensionale perchè non dipende dal codice del programma.
  \end{itemize}
\end{example}

\subsubsection{Esempi}
\begin{example}
  Consideriamo l'insieme dei programmi che divergono sempre:
  \[
    \begin{aligned}
      A &= \left\{ \left. x \;\right|\; W_x\footnote{\( W_x \stackrel{def}{=} dom \varphi_x
          = \left\{ \left. y \;\right|\; \varphi_x(y) \downarrow \right\}\) } = \varnothing \right\}\\
        &= \left\{ \left. x \;\right|\; \forall y \;.\; \varphi_x(y) \uparrow \right\}
    \end{aligned}
  \] 
  Dimostriamo che \( A \) non è ricorsivo tramite il teorema di Rice.

  \vspace{1em}
  \noindent
  \begin{enumerate}
    \item Dimostriamo che \( A \) è estensionale:
      \begin{itemize}
        \item Prendiamo \( z \in A \stackrel{def A}{\implies} W_z = \varnothing \equiv
          \forall y \;.\; \varphi_z(y) \uparrow \)
        \item Prendiamo \( w \in \mathbb{N} \;.\; \varphi_w = \varphi_z \implies
          W_w = W_z = \varnothing \) perchè:
          \[
            \forall y \;.\; \varphi_w(y) = \varphi_z(y) \uparrow
          \] 
      \end{itemize}
      Quindi per definizione di \( A \) \( w \in A \) 

    \item Dimostriamo che \( A \) è non banale, cioè:
      \[
        \begin{aligned}
          A &\neq \varnothing \; (\exists x \in A) \wedge \\
          A &\neq \mathbb{N} \; (\exists x \notin A)
        \end{aligned}
      \] 
      \begin{itemize}
        \item 
          \(
            f(x) = \lambda x. x
          \) 
          Questa funzione è ricorsifva, quindi esiste una macchina di Turing che la calcola:
          \[
            \exists n \;.\; \varphi_n = f \implies W_n = \mathbb{N} \neq \varnothing \implies
            n \notin A \implies A \neq \mathbb{N}
          \] 

        \item \( f(x) = \lambda x. \uparrow \) (while true do x = x) Questa funzione
          è ricorsiva quindi, esiste una macchina di Turing che la calcola:
          \[
            \exists m \;.\; \varphi_m = f \implies W_m = \varnothing \implies m \in A \implies
            A \neq \varnothing
          \] 
      \end{itemize}
  \end{enumerate}
...
\end{example}
\begin{example}
  Consideriamo il complemento dell'esempio precedente:
  \[
    B = \left\{ \left. x \;\right|\; W_x \neq \varnothing \right\}
  \] 
  Se \( A \) fosse ricorsivo, per il teorema di Post anche \( \bar{A} = B \) sarebbe ricorsivo.
  Siccome \( A \) non è ricorsivo, neanche \( B \) lo è.
\end{example}

\subsection{Dimostrare che un insieme è ricorsivamente enumerabile}
Per dimostrare che un insieme è RE si fornisce l'algoritmo che implementa la
funzione semicaratteristica.

\subsection{Dimostrare che un insieme non è ricorsivamente enumerabile}
Per dimostrare che un insieme non è RE si usa il \textbf{teorema di Post}
\[
  A \text{ è ricorsivo } \iff A \wedge \bar{A} \text{ sono RE}
\] 

\subsection{Riducibilità funzionale}
Si usa per "ridurre" un problema sconosciuto ad un problema noto.
L'inclusione insiemistica non permette di ereditare proprietà di ricorsione, ad
esempio:
\[
  \underset{RIC}{\varnothing} \subseteq \underset{RE}{K} \subseteq \underset{RIC}{\mathbb{N}}
\] 

\vspace{1em}
\noindent
Consideriamo un insieme \( A \) di cui non conosciamo la ricorsività e un
insieme \( B \) con ricorsività nota (ad esempio \( B \) ricorsivo). Allora:
\[
  \exists f_B(x) =
  \begin{cases}
    1 & \text{se } x \in B \\
  ...
  \end{cases}
\] 
Se si riesce a prendere un elemento \( x \in A \) e trasformarlo in un elemento
di B \( h(x) \in B \), allora si può prendere l'elemento \( h(x) \) e calcolare
\( f_B(h(x)) \) per sapere se \( x \in A \):
\[
   x \leadsto h(x) \leadsto f_B(h(x)) =
  \begin{cases}
    1 & \text{se } h(x) \in B \implies x \in A \\
    0 & \text{se } h(x) \notin B \implies x \notin A
  \end{cases}
\] 
La riducibilità funzionale da \( A \) a \( B \) è definita come
la funzione \textbf{totale} \( h: \mathbb{N} \to \mathbb{N} \) tale che
\( x \in A \iff h(x) \in B \):
\[
  \implies (B \text{ ricorsivo } \implies A \text{ ricorsivo})
\] 
Quindi \( f \circ h \) è la funzione caratteristica di \( A \) e
quindi \( A \) è ricorsivo.

\begin{definition}
  Siano \( A, B \subseteq \mathbb{N} \). \( A \) si \textbf{si riduce} (funzionalmente)
  a \( B \) (denotato \( A \preceq_f B \)) se esiste \( f: \mathbb{N} \to \mathbb{N} \) 
  \textbf{totale ricorsiva} tale che:
  \[
    x \in A \iff f(x) \in B
  \] 
\end{definition}
...
\begin{example}
  ...
  coppie programma-input tali che il programma \( x \) termina sull'input \( y \).
  \[
    \begin{aligned}
      K_2 &= \left\{ \left. \left<x, y \right> \;\right|\; y \in W_x \right\}
          &= \left\{ \left. \left<x, y \right> \;\right|\; \varphi_x(y) \downarrow \right\}\\
    \end{aligned}
  \] 
  Ci chiediamo in che classe di ricorsività sia \( K_2 \).

  \vspace{1em}
  \noindent
  Sappiamo che \( K_2 \subseteq \mathbb{N} \times \mathbb{N} \), e generalizzando
  abbiamo che:
  \[
    x \in K \iff \left<x, x \right> \in K_2
  \] 
  Questa è una riduzione funzionale da \( K \) a \( K_2 \):
  \[
    f(x) = \lambda x. \left<x, x \right>
  \] 
  Quindi:
  \[
     x \in K \iff f(x) \in K_2 \implies k \preceq_f K_2
  \] 
  Siccome \( K \) è ricorsivamente enumerabile ma non ricorsivo, allora
  se \( K_2 \) fosse ricorsivo anche \( K \) lo sarebbe, quindi \( K_2 \)
  non è ricorsivo.
  Possiamo costruire l'algoritmo della semicaratteristica di \( K_2 \), quindi
  \( K_2 \) è RE non ricorsivo.
\end{example}

\subsubsection{Insieme completo}
\begin{definition}
  Un insieme \( C \in RE \) è completo se per ogni \( A \in RE \) \( A \) si riduce
  a \( C \):
  \[
    \forall a \in RE \;.\; A \preceq_f C
  \] 

  \vspace{1em}
  \noindent
  La riducibilità funzionale è:
  \begin{itemize}
    \item \textbf{Transitiva}:
      \[
        A \preceq_f B \wedge B \preceq_f C \implies A \preceq_f C
      \] 
      Quindi:
      \[
        \begin{aligned}
    ...
    \item \textbf{Riflessiva}:
      ...
      \[
        x \in A \iff x \in A
      \] 
      e la funzione di riduzione è l'identità:
      \[
        f(x) = \lambda x. x
      \] 
  \end{itemize}
...
\begin{theorem}
  L'insieme \( K_2 \) è completo per la classe RE.

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione}: Sia \( A \in RE \), allora esiste un algoritmo di cui
  \( A \) è il dominio:
  \[
    \exists y_0 \;.\; A = W_{y_0}
  \] 
  \[
    x \in A = W_{y_0} \underset{def W_{y_0}}{\iff} \varphi_{y_0}(x) \downarrow
    \underset{def K_2}{\iff} \left<y_0, x\right> \in K_2
  \] 
  Quindi la funzione di riduzione funzionale è:
  \[
    f_{y_0} = \lambda x. \left<y_0, x\right> \implies A \preceq_f K_2
  \] 
  cioè:
  \[
    x \in A \iff f_{y_0}(x) \in K_2
  \] 
  Vista la genericità di \( A \), vale per ogni insieme \( A \) in RE,
  quindi \( K_2 \) è completo.
\end{theorem}
\vspace{1em}
\noindent
\begin{theorem}
  ...
  \noindent
  \textbf{Dimostrazione:}
  (\( \forall A \in RE \;.\; A \preceq_f K_2 \))
  Siccome \( \preceq_f \) è transitiva se dimostriamo che \( K_2 \preceq_f K \)
  allora \( K \) è completo:
  \[
    \forall A \;.\; A \preceq_f K_2 \preceq_f K \implies K \text{ completo}
  \] 
  \[
    \left<x, y \right> \in K_2 \iff \varphi_x(y) \downarrow
  \] 
  Definiamo la funzione \( \psi \) con un numero di input pari agli input dell'insieme
  di partenza più uno:
  \[
    \psi(x, y, z) =
    \begin{cases}
    ...
    \end{cases}
  \] 
  Questa funzione è la caratteristica di \( K_2 \) con un input in più \( z \).
  Esiste un algoritmo che calcola \( \psi \) e quindi \( \psi \) è parziale ricorsiva
  e possiamo applicare il teorema smn:
  \[
    \exists g \text{ totale ricorsiva } \quad g: \mathbb{N} \times \mathbb{N} \to \mathbb{N}
  \] 
  tale che:
  \[
    \psi(x,y,z) = \varphi_{g(x, y)}(z)
  \] 
  si specializza sui primi due input. Ci chiediamo se \( g \) è una funzione di
  riduzione funzionale da \( K_2 \) a \( K \):
  \[
   \left<x, y \right> \in K_2 \iff g(x, y) \in K
  \] 
  \[
    \begin{aligned}
      \left<x, y \right> \in K_2 &\underset{def K_2}{\implies} \varphi_x(y) \downarrow\\
                                 &\underset{def \psi}{\implies} \psi(x, y, z) \downarrow\\
                                 &\underset{smn}{\implies} \forall z \;.\; \varphi_{g(x, y)}(z) \downarrow\\
                                 &\underset{z = g(x,y)}{\implies} \varphi_{g(x, y)}(g(x, y)) \downarrow\\
                                 &\underset{def K}{\implies} g(x, y) \in K
    \end{aligned}
  \] 
  Quindi:
  \[
    \begin{aligned}
      \left<x, y \right> \notin K_2 &\underset{def K_2}{\implies} \varphi_x(y) \uparrow\\
                                    &\underset{def \psi}{\implies} \forall z \;.\; \psi(x, y, z) \uparrow\\
                                    &\underset{smn}{\implies} \forall z \;.\; \varphi_{g(x, y)}(z) \uparrow\\
                                    &\underset{z = g(x,y)}{\implies} \varphi_{g(x, y)}(g(x, y)) \uparrow\\
                                    &\underset{def K}{\implies} g(x, y) \notin K
    \end{aligned}
  \] 
\end{theorem}

\subsubsection{Proprietà della riducibilità funzionale}
\begin{enumerate}
  \item \( A \) si riduce a \( B \) se e solo se \( \bar{A} \) si riduce a \( \bar{B} \):
    \[
      A \preceq_f B \iff \bar{A} \preceq_f \bar{B}
    \]
    \[
      x \in A \iff f(x) \in B \equiv x \notin \bar{A} \iff f(x) \notin \bar{B}
    \] 

  \item Se \( A \) si riduce a \( B \) e \( B \) è ricorsivamente enumerabile,
    allora anche \( A \) è ricorsivamente enumerabile: 
    \[
      A \preceq_f B \implies \exists f: \mathbb{N} \to \mathbb{N} \text{ totale ricorsiva }
      \;.\; x \in A \iff f(x) \in B
    \] 
    \[
      B \in RE \implies \exists \psi_B(x) =
      \begin{cases}
        1 & \text{se } x \in B \\
        \uparrow & \text{se } x \notin B
      \end{cases}
    ...
    \[
      ...
      \begin{cases}
        ...
        \uparrow & \text{altrimenti}
      \end{cases}
      \underset{\text{per riduzione}}{=}
      \begin{cases}
        1 & \text{se } x \in A \\
        \uparrow & \text{altrimenti}
      \end{cases}
    \] 
    è parziale ricorsiva in quanto \( f \) è totale. Ci dobbiamo assicurare che diverge solo
    quando la condizione di terminazione è falsa.
    \( \psi  \) è la semicaratteristica di \( A \) e quindi \( A \in RE \).

  \item Se \( A \) si riduce a \( B \) e \( B \) è ricorsivo,
    allora anche \( A \) è ricorsivo:
    \[
      A \preceq_f B \implies \exists f: \mathbb{N} \to \mathbb{N} \text{ totale ricorsiva }
      \;.\; x \in A \iff f(x) \in B
    \] 
    \[
      B \in RIC \implies \exists f_B(x) =
      \begin{cases}
        1 & \text{se } x \in B \\
        0 & \text{se } x \notin B
      \end{cases}
    ...
    \[
      ...
      \end{cases}
    \] 
    è totale ricorsiva in quanto \( f \) è totale. \( h \) è la caratteristica di \( A \)
    e quindi \( A \in RIC \).
\end{enumerate}

\subsection{Insiemi creativi}
\begin{definition}
  Un insieme \( I \subseteq \mathbb{N} \) è \textbf{creativo} se è \textbf{ricorsivamente
  enumerabile} ed è il complemento di un insieme \textbf{produttivo}
\end{definition}
Gli insiemi creativi coincidono esattamente con gli insiemi completi, cioè \( A \) 
è completo se:
\[
  \forall B \in RE \;.\; B \preceq_f A
\] 

\subsection{Insiemi produttivi}
Il complemento di un insieme produttivo può essere:
\begin{itemize}
  \item Ricorsivamente enumerabile, e allora l'insieme produttivo è creativo
  \item Non ricorsivamente enumerabile
\end{itemize}
Quindi:
\[
  A \in RE \leadsto \psi_A(x) =
  \begin{cases}
    1 & \text{se } x \in A \\
    \uparrow & \text{se } x \notin A
  \end{cases}
\] 
\[
  \bar{A} \notin RE \leadsto \psi_{\bar{A}}(x) =
  \begin{cases}
    \uparrow & \text{se } x \in \bar{A} \\
    1 & \text{se } x \notin \bar{A}
  \end{cases}
\] 
Questa \textbf{NON} è la semicaratteristica di \( \bar{A} \) perchè diverge quando l'imput
appartiene a \( \bar{A} \). Ma la non appartenenza a \( \bar{A} \) (\( x \in A \)) è
\textbf{semidecidibile}, cioè esiste un algoritmo che si ferma quando l'input
appartiene a \( A \).
Possono esistere insiemi in cui nessuno dei due test (\( \in, \notin  \)) è semidecidibile.
Quindi:
\[
  \begin{aligned}
    A &\notin RE \implies \bar{A} \in RE \vee \bar{A} \notin RE\\
    A &\in RE \implies \bar{A} \notin RE
  \end{aligned}
\] 
\begin{example}
  L'insieme:
  \[
    \left\{\left. x \;\right|\; W_x = \varnothing \right\}
  \] 
  non è ricorsivamente enumerabile, ma il suo complemento:
  \[
    \left\{\left. x \;\right|\; W_x \neq \varnothing \right\}
  \] 
...
\begin{example}
  L'insieme:
  \[
    \left\{\left. x \;\right|\; W_x = \mathbb{N} \right\}
  \] 
  non è ricorsivamente enumerabile perchè:
  \[
    \forall n \in \mathbb{N} \;.\; \varphi_x(n) \downarrow
  \] 
  ...
  non è ricorsivamente enumerabile perchè:
  \[
    \exists y \in \mathbb{N} \;.\; \varphi_x(y) \uparrow
  \] 
\end{example}

\begin{definition}
  L'insieme \( A \subseteq \mathbb{N} \) è \textbf{produttivo} se esiste una
  funzione \textbf{totale ricorsiva} tale che:
  \[
    \forall x \in \mathbb{N} \;.\; \left( W_x \subseteq A \implies f(x) \in A \setminus W_x \right) 
  \] 
  Per qualunque tentativo di enumerare gli elementi di \( A \) con un algoritmo
  \( \varphi_x \), la funzione \( f \) trasforma \( x \in A \) in un elemento
  non enumerato da \( W_x \).
\end{definition}
Quindi:
\[
  \begin{aligned}
    A \text{ produttivo } &\implies \bar{A} \text{ produttivo} \vee \bar{A} \text{ creativo}\\
    A \text{ creativo } &\implies \bar{A} \text{ produttivo }
  \end{aligned}
\] 
L'insieme \( K \) è creativo, quindi il suo complemento \( \bar{K} \) è produttivo.

\subsubsection{Insieme \texorpdfstring{\( \bar{K} \)}{K} produttivo}
Per dimostrare che un insieme è produttivo bisogna costruire la funzione
\( f \) che soddisfa la definizione di insieme produttivo. Per dimostrare
che \( \bar{K} \) è produttivo consideriamo la funzione identità:
\[
  \begin{aligned}
    f &= id\\
    f(x) &= x
  \end{aligned}
\] 
\textbf{Dimostrazione}: dimostriamo che \( \bar{k} \) è produttivo con \( f = id \).
Prendiamo \( x \in \mathbb{N} \), prendiamo \( W_x \) supponendo che \( W_x \subseteq \bar{K} \),
cioè esiste almeno un tale \( W_x \) perchè \( A \) non può essere vuoto.
Sapendo che \( f(x) = x \), allora \( \varphi_x(x) \) può terminare?
No perchè per definizione \( x \in W_x  \), quindi:
\[
  \begin{aligned}
    x &\in W_x \subseteq \bar{K}\\
      &\implies x \in \bar{K} \text{ ovvero } \varphi_x(x) \uparrow\\
      &\implies x \in \bar{K} \land x \notin W_x \implies x \in \bar{K} \setminus W_x\\
      &\implies \text{ per def. } \bar{K} \text{ è produttivo}
  \end{aligned}
\] 

\vspace{1em}
\noindent
Per definizione di insieme creativo (\( A \) è creativo se \( A \in RE \) e
\( \bar{A} \) è produttivo)
abbiamo dimostrato che \( K \in RE \) e anche che \( K \) è il complemento
di \( \bar{K} \) che è produttivo, quindi \( K \) è \textbf{creativo}. 

\begin{theorem}[Teorema di Myhill]
  L'insieme \( A \) è creativo se e solo se \( A \) è completo, cioè:
  \[
    A \text{ creativo } \iff \forall B \in RE \;.\; B \preceq_f A
  \] 
  ...
  \begin{itemize}
    \item \( \Leftarrow \):\\
      Sia \( A \) completo (di conseguenza anche \( RE \)), allora per definizione di completezza:
      \[
        \forall B \in RE \;.\; B \preceq_f A
      \] 
      Essnedo \( A \) completo, si ha che:
      \[
        K \in RE \implies K \preceq_f A \underset{prop.\;riduzione}{\iff}
        \bar{K} \preceq_f \bar{A}
      \] 
      \[
        \bar{K} \text{ produttivo} \implies \bar{A} \text{ produttivo} \quad \text{ (Per il teorema \ref{thm:produttivo}.1)}
      \] 
      Quindi se \( A \) è, cioè se \( A \) è \( RE \) e \( \bar{A} \) è produttivo, allora
      \( A \) è creativo.

    \item \( \Rightarrow \): Non dimostrato in questo corso.
  \end{itemize}
\end{theorem}

\begin{theorem}
  Siano \( A, B \subseteq \mathbb{N} \) allora: 
\label{thm:produttivo}
  \begin{enumerate}
    \item  Se \( A \) è produttivo e \( A \preceq_f B \),
      allora \( B \) è produttivo.

    \item Se \( A \) è creativo, \( A \preceq_f B \) e \( B \in RE \),
      allora \( B \) è creativo. Dimostrare la sola riduzione non è sufficiente
      per ereditare la proprietà di creatività.
      \[
        k \preceq_f A \equiv \bar{K} \preceq_f \bar{A}
      \] 
      e si ha che:
      \begin{itemize}
        \item \( \bar{K} \preceq_f \bar{A} \implies \bar{A} \text{ produttivo} \)
        \item \( K \preceq_f A \) e:
          \begin{itemize}
            \item Se \( A \) è \( RE \), allora \( A \) è creativo
            \item Se \( A \) non è \( Re \), allora \( \bar{K} \preceq_f A \) e
              \( A \) è produttivo
          \end{itemize}
      \end{itemize}
  \end{enumerate}
  ...
  \textbf{Dimostrazione}:
  \begin{enumerate}
    \item Sia \( A \) produttivo, allora:
      \[
        \exists f_A \;.\; \forall x \; W_x \subseteq A \implies f_A(x) \in A \setminus W_x
      \] 
      \[
        A \preceq_f B \quad \exists f \;.\; x \in A \iff f(x) \in B
      \] 
      \[
        \psi(xy) =
        \begin{cases}
        ...
        \end{cases}
      \] 
      Questa è una condizione semidecidibile, quindi \( \psi(xy) \) è parziale ricorsiva
      perchè diverge \textbf{solo} quando la condizione è falsa. Di conseguenza si
      può applicare il teorema smn:
      \[
        \exists g \text{ totale ricorsiva } \;.\; \psi(xy) = \varphi_{g(x)}(y)
      \] 

      \vspace{1em}
      \noindent
      Per dimostrare che \( B \) è produttivo dobbiamo definire \( f' \) tale
      che:
      \[
        W_x \subseteq B \implies f'(x) \in B \setminus W_x
      \] 
      \begin{itemize}
        \item 
          Prendiamo \( W_{g(x)} \subseteq A \) e applichiamo la funzione \( f_A \):
          \[
            f_A(g(x)) \in A \setminus W_{g(x)} \text{ (per produttività di A)}
          \] 

        \item Prendiamo \( W_x \subseteq B \) e applichiamo la funzione di riduzione \( f \) a
          \( f_A(g(x)) \). La funzione \( f \) mappa elementi da \( A \) a \( B \)
          e bisogna capire se il risultato appartiene a \( B \setminus W_x \)
          o a \( W_x \).

          Supponiamo (per assurdo) che \( f(f_A(g(x))) \in W_x \), allora:
          \[
            \begin{aligned}
              f(f_A(g(x))) \in W_x &\underset{def\; \psi}{\implies} \psi(x, f_A(g(x))) = 1\\
                                   &\underset{smn}{\implies} \varphi_{g(x)}(f_A(g(x))) = \downarrow\\
                                   &\underset{def\; W_{g(x)}}{\implies} f_A(g(x)) \in W_{g(x)} \text{ Assurdo}
            \end{aligned}
          \] 
          Quindi:
          \[
            f(f_A(g(x))) \in B \setminus W_x \implies B \text{ è produttivo con }
            f' = f \circ f_A \circ g
          \] 
      \end{itemize}

    \item A è creativo solo se \( \bar{A} \) è produttivo e \( RE \):
      \[
        A \text{ creativo} \iff \bar{A} \text{ produttivo } \land \bar{A} \in RE
      \] 
       e 
      \[
        A \preceq_f B \implies \bar{A} \preceq_f \bar{B}
      \] 
      Di conseguenza \( \bar{B} \) è produttivo e anche \( RE \), quindi \textbf{è creativo}.
  \end{enumerate}
\end{theorem}

\begin{theorem}
  Se \( A \) è produttivo, allora \( A \notin RE \).

  \vspace{1em}
  \noindent
  \textbf{Dimostrazione}: Supponiamo (per assurdo) che \( A \in RE \) e sia
  \( f \) la funzione di produttività di \( A \):
  \[
    \exists f \;.\; W_x \subseteq A \implies f(x) \in A \setminus W_x
  \] 
  Per definizione di \( A \in RE \):
  \[
    \begin{aligned}
      \exists u \in \mathbb{N} \;.\; A = W_u &\implies W_u \subseteq A \\
                                             &\underset{\text{def produttività}}{\implies} f(u) \in A \setminus W_u = \varnothing \text{ Assurdo}
    \end{aligned}
  \] 
\end{theorem}

\subsection{Riassunto di classificazione degli insiemi}
\subsubsection{Insiemi ricorsivi}
L'unico modo per dimostrare che un insieme è ricorsivo è:
\begin{itemize}
  \item Dimostrazione per definizione:\\
    Bisogna fornire l'algoritmo \textbf{decidibile}
...
\subsubsection{Insiemi ricorsivamente enumerabili}
Per dimostrare che un insieme è ricorsivamente enumerabile si possono usare i seguenti
moetodi:
\begin{itemize}
  \item Dimostrazione per definizione:\\
    Bisogna fornire l'algoritmo \textbf{semidecidibile}
    della funzione semicaratteristica:
    \[
      \psi_A(x) =
      \begin{cases}
        1 & \text{se } x \in A \\
  ...
  \item Dimostrazione per riduzione funzionale da un insieme noto RE:
    \[
      A \preceq_f K \land K \in RE \implies A \in RE
    \] 
\end{itemize}

\vspace{1em}
\noindent
Per dimostrare che un insieme non è ricorsivamente enumerabile si può usare il
teorema di Post:
\[
  A \text{ non è ricorsivo } \implies A \notin RE \vee \bar{A} \notin RE
\]

\subsubsection{Insiemi creativi}
L'unico modo per dimostrare che un insieme è creativo è:
\begin{itemize}
  \item Dimostrazione per definizione: \\
    Bisogna dimostrare che l'insieme è ricorsivamente enumerabile e che il suo
    complemento è produttivo:
    \[
      A \in RE \land \bar{A} \text{ produttivo } \equiv \bar{K} \preceq_f \bar{A} \equiv K \preceq_f A
      \implies A \text{ creativo}
    \]
    Questo dimostra anche che \( \bar{A} \) è produttivo
\end{itemize}
\textbf{Gli insiemi creativi non sono ricorsivi}.

\subsubsection{Insiemi produttivi}
L'unico modo per dimostrare che un insieme è produttivo è:
\begin{itemize}
  \item Dimostrazione per teorema \ref{thm:produttivo}.1:
    \[
      \bar{K} \preceq_f A \implies A \text{ produttivo}
    \] 
    Per quanto riguarda \( K \preceq_f \bar{A} \) non dice nulla su \( \bar{A} \).
\end{itemize}
\textbf{Gli insiemi produttivi non sono in RE}.


\subsection{Esercizi}
\begin{exercise}
  Consideriamo l'insieme dei programmi che terminano sul quadrato del loro input:
  \[
    A = \left\{\left. x \;\right|\; \varphi_x(x^2) \downarrow \right\}
  \] 
  \begin{enumerate}
    \item 
      Questo insieme è intuitivamente ricorsivamente enumerabile. Per dimostrarlo
      bisogna costruire l'algoritmo per \( \psi_A(x) \) che si ferma solo se
      l'input appartiene ad \( A \):
      \[
        \psi_A(x) =
        \begin{cases}
          1 & \text{se } x \in A\\
...
\begin{lstlisting}[language=Python]
input(x) {
  y = x^2; # funzione x^2 e' totale ricorsiva
  costruisci phi_x; # esiste una procedura totale e ricorsiva per costruire phi_x da x
  stop = false;
  while !stop {
    esegui il prossimo passo di phi_x(y);
...
\end{lstlisting}
    Questo dimostra che \( A \in RE \).

  \item Dimostrare che \( K \preceq_f A \). Per farlo bisogna costruire una
    funzione totale ricorsiva \( f \) tale che:
    \[
      \exists f \text{ totale ricorsiva } \;.\; x \in K \iff f(x) \in A
    \]
    La funzione \( f(x) \) viene costruita a partire da una \( g \) del teorema smn.
    ...
    \[
      ...
      \end{cases}
    \] 
    La funzione \( \psi  \) è parziale ricorsiva perchè la condizione di terminazione
    è l'appartenenza ad un insieme ricorsivamente enumerabile. Di conseguenza
    si può applicare il teorema smn:
    \[
      \exists g \text{ totale ricorsiva } \;.\; \psi(xy) = \varphi_{g(x)}(y)
    \] 
    \begin{itemize}
      \item Se \( x \in K \) allora:
        \[
          \begin{aligned}
            x \in K &\underset{def\; \psi}{\implies} \forall y \;.\; \psi(xy) = 1\\
                    &\underset{smn}{\implies} \forall y \;.\; \varphi_{g(x)}(y)\downarrow\\
                    &\underset{y = g(x)^2}{\implies} \varphi_{g(x)}(g(x)^2)\downarrow\\
                    &\underset{def\; A}{\implies} g(x) \in A
          \end{aligned}
        \] 
      ...
      \item Se \( x \notin K \) allora:
        \[
          \begin{aligned}
            x \notin K &\underset{def\; \psi}{\implies} \forall y \;.\; \psi(xy)\uparrow\\
                        &\underset{smn}{\implies} \forall y \;.\; \varphi_{g(x)}(y) \uparrow\\
                        &\underset{y = g(x)^2}{\implies} \varphi_{g(x)}(g(x)^2) \uparrow\\
                        &\underset{def\; A}{\implies} g(x) \notin A
          \end{aligned}
        \]
    \end{itemize}
    Abbiamo dimostrato che \( f(x) = g(x) \) è la funzione di riduzione funzionale
    da \( K \) ad \( A \):
    \[
      x \in K \iff f(x) \in A
    \]
    Quindi \( A \) è creativo e \( \bar{A} \) è produttivo.
  \end{enumerate}
\end{exercise}
...
\begin{exercise}
  Consideriamo l'insieme:
  \[
    A = \left\{\left. x \;\right|\; \varphi_x(x^2) = 3 \right\}
  \] 
  \begin{enumerate}
    \item Dimostrare che \( A \in RE \). Per farlo bisogna costruire l'algoritmo
      della semicaratteristica di \( A \):
      \[
        \psi_A(x) =
        \begin{cases}
          1 & \text{se } x \in A\\
    ...
    \item Dimostrare che \( K \preceq_f A \). Per farlo bisogna costruire una
      ...
      \begin{itemize}
        \item Se \( x \in K \) allora:
          \[
            \begin{aligned}
              x \in K &\underset{def\; \psi}{\implies} \forall y \;.\; \psi(xy) = 3\\
                      &\underset{smn}{\implies} \forall y \;.\; \varphi_{g(x)}(y) = 3\\
                      &\underset{y = g(x)^2}{\implies} \varphi_{g(x)}(g(x)^2) = 3\\
                      &\underset{def\; A}{\implies} g(x) \in A
            \end{aligned}
          \] 
...
\begin{exercise}
  Consideriamo l'insieme:
  \[
    A = \left\{\left. x \;\right|\; \varphi_x(x^2) = x^3 \right\}
  \] 
  \begin{enumerate}
    \item Dimostrare che \( A \in RE \). Per farlo bisogna costruire l'algoritmo
      della semicaratteristica di \( A \):
...
input(x) {
  y = x^2; # funzione x^2 e' totale ricorsiva
  z = x^3; # funzione x^3 e' totale ricorsiva
  costruisci phi_x; # esiste una procedura totale e ricorsiva per costruire phi_x da x
  stop = false;
  while !stop {
    esegui il prossimo passo di phi_x(y);
...
\end{lstlisting}
    ...
    \item Dimostrare che \( K \preceq_f A \). Per farlo bisogna costruire una
      ...
      \vspace{1em}
      \noindent
      Bisogna ricreare la \( x \) partendo dall'input \( y \) per poter calcolare \( x^3 \):
      \[
        \psi(xy) = 
        \begin{cases}
          z^3 & \text{se } \overbrace{x \in K}^{\text{parziale}}
          \land \overbrace{\exists z \;.\; y = z^2}^{\text{totale}}\\
          \uparrow & \text{altrimenti}
        \end{cases}
      \] 
      Dobbiamo dimostrare che \( \psi  \) è parziale ricorsiva costruendo il procedimento
      che calcola \( \psi  \) in modo tale che termini sui valori che rendono vera la
      condizione (l'ordine non è importante perchè c'è un \( \land  \)):
\begin{lstlisting}[language=Python]
input(x, y) {
  z = 0;
  w = 0; # z^2
  # se non troviamo z tale che y = z^2, il ciclo diverge, ma la condizione e' falsa, quindi va bene
  while y != w {
    z = z + 1;
    w = z^2;
  }
  ...
  # Se entrambi i cicli terminano, allora la condizione e' vera
  return z^3;
}
\end{lstlisting}
     Quindi \( \psi \) è parziale ricorsiva e si può applicare il teorema smn:
      \[
        \exists g \text{ totale ricorsiva } \;.\; \psi(xy) = \varphi_{g(x)}(y)
      \] 
      \begin{itemize}
        \item Se \( x \in K \) allora:
          \[
            \begin{aligned}
              x \in K &\underset{def\; \psi}{\implies} \psi(xy)\downarrow = z^3 \iff 
              \exists z \;.\; y = z^2\\
                      &\underset{smn}{\implies} \varphi_{g(x)}(y)\downarrow = z^3 \iff
              \exists z \;.\; y = z^2\\
                      &\underset{\underset{z = g(x)}{y = g(x)^2}}{\implies} \varphi_{g(x)}(g(x)^2)\downarrow = g(x)^3\\
                      &\underset{def\; A}{\implies} g(x) \in A
            \end{aligned}
          \]
...
\begin{exercise}
  ...
  \[
    \begin{aligned}
      A &= \left\{\left. x^4 \;\right|\; \varphi_x(x^2) = x^3 \right\}\\
        &= \left\{\left. y \;\right|\; \exists x \;.\; y = x^4 \land \varphi_x(y^2) = y^3 \right\}
    \end{aligned}
  \] 
  \begin{enumerate}
    \item L'insieme \( A \) rimane \( RE \) per la stessa dimostrazione dell'esercizio
      precedente. La funzione semicaratteristica è:
\begin{lstlisting}[language=Python]
input(y) {
  x = 0; # base x^4
  z = 0; # z = x^4
  while y != z {
    x = x + 1;
    z = x^4; # se y = x^4, il ciclo termina e x e' la base
  }
  # se il while termina, y = x^4
  ...
  stop = false;
  while !stop {
    esegui il prossimo passo di phi_x(w);
    if phi_x(w) ha terminato {
      if phi_x(w) == z {
        stop = true;
      }
    }
  }
  # se il while termina, phi_x(x^2) == x^3
...
\end{lstlisting}
    ...
    \item Dimostrare che \( K \preceq_f A \). Per farlo bisogna costruire una
      ...
      \vspace{1em}
      \noindent
      La stessa funzione dell'esercizio precedente funziona anche qui perchè
      l'unica differenza è la condizione sull'input:
      \[
        \psi(xy) = 
        \begin{cases}
          z^3 & \text{se } \overbrace{x \in K}^{\text{parziale}}
        ...
        \end{cases}
      \] 
      La dimostrazione che \( \psi  \) è parziale ricorsiva è uguale a quella
      dell'esercizio precedente. Quindi si può applicare il teorema smn:
      \[
        \exists g \text{ totale ricorsiva } \;.\; \psi(xy) = \varphi_{g(x)}(y)
      \] 
      \begin{itemize}
        \item Se \( x \in K \) allora:
          \[
            \begin{aligned}
              x \in K &\underset{def\; \psi}{\implies} \psi(xy) = z^3 \iff 
              \exists z \;.\; y = z^2\\
                      &\underset{smn}{\implies} \varphi_{g(x)}(y) = z^3 \iff
              \exists z \;.\; y = z^2\\
                      &\underset{\underset{z = g(x)}{y = g(x)^2}}{\implies} \varphi_{g(x)}(g(x)^2) = g(x)^3\\
                      &\underset{def\; A}{\implies} g(x)^4 \in A
            \end{aligned}
          \]
        \item Se \( x \notin K \) allora:
          \[
            \begin{aligned}
              x \notin K &\underset{def\; \psi}{\implies} \forall y \;.\; \psi(xy)\uparrow\\
                         &\underset{smn}{\implies} \forall y \;.\; \varphi_{g(x)}(y) \uparrow\\
                         &\underset{y = g(x)^2}{\implies} \varphi_{g(x)}(g(x)^2) \uparrow\\
                         &\underset{def\; A}{\implies} g(x)^4 \notin A
            \end{aligned}
          \]
      \end{itemize}
      Abbiamo dimostrato che \( f(x) = g(x)^4 \) è la funzione di riduzione funzionale
      da \( K \) ad \( A \):
      \[
        x \in K \iff f(x) \in A
      \]
  ...
  \textbf{Osservazione}: Tutto questo vale perchè \( x^4 \) è una funzione crescente
  e invertibile su \( \mathbb{N} \), quindi esiste sempre un unico \( x \) input
  per ogni \( y \) tale che \( y = x^4 \).
\end{exercise}
