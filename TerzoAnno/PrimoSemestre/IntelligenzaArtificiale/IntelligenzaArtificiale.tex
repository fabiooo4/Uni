...
\subsection{MDP parzialmente osservabili}
I POMDP (Partially Observable Markov Decision Processes) sono un modello di osservazione
\( O(s, e) \) che definisce la probabilità che l'agente ottiene l'evidenza \( e \) quando
si trova nello stato \( s \). L'agente non conosce lo stato corrente.

\begin{theorem}
  La policy ottima in un POMDP è una funzione \( \pi(b) \) dove \( b \) è il
  \textbf{belief state}, cioè una distribuzione di probabilità sugli stati
  possibili in cui l'agente può trovarsi.
\end{theorem}

\noindent
Si può convertire un POMDP in un MDP nello spazio dei belief state, dove \( T(b, a, b') \) 
è la probabilità che il nuovo belief state sia \( b' \) sapendo che il belief state
corrente è \( b \) e l'azione eseguita è \( a \).

\section{Machine learning}
Il machine learning consiste nel creare modelli partendo da un insieme di dati. Ci
sono diversi tipi di learning:
\begin{itemize}
...
\subsection{Concetti di base e terminologia}
\begin{itemize}
  \item \textbf{Variabili di input}:
    \begin{itemize}
      \item Variabili indipendenti o predittori o features
      \item \( X \), o quando ci sono più variabili \( X_1, \ldots, X_p \) 
    \end{itemize}

  \item \textbf{Variabili di output}:
    \begin{itemize}
      \item Variabili dipendenti o risposte
      \item \( Y \)
    \end{itemize}

  \item \textbf{Variabili quantitative}: sono variabili con valori numerici, ad esempio
    la temperatura, l'altezza, il peso, ecc.

  \item \textbf{Variabili qualitative} (o categoriche): sono variabili con valori in
    una di \( K \) classi diverse, ad esempio il genere, la razza, la specie, ecc.

  \item \textbf{Regressione}: predizione di una variabile quantitativa
  \item \textbf{Classificatione}: predizione di una variabile qualitativa
\end{itemize}

\subsection{Supervised learning}
È l'approccio più utilizzato e consiste nell'addestrare un modello a partire
da un insieme di dati etichettati.
\begin{example}
  Prendiamo ad esempio la classificazione di numeri scritti a mano.
  \begin{itemize}
    \item Training set:
      \begin{itemize}
        \item Immagini conosciute \( x \) che rappresentano una certa cifra
          \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\textwidth]{supervised_learning}
            \caption{Esempio di immagini di cifre scritte a mano}
          \end{figure}
        \item Etichette \( t \)  che rappresenta la cifra
      \end{itemize}

    \item Training: Costruisce un modello che mappa immagini a cifre \( y(x) \) 

    \item Test set: \textbf{Nuove} immagini \textbf{senza etichette}.
    \item Testing: Applicazione del modello al test set
  \end{itemize}
\end{example}
...
\begin{example}
  Prendiamo ad esempio il raggruppamento di cellule basato sull'espressione genica.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{unsupervised_learning}
    \caption{Esempio di clustering di cellule}
  \end{figure}
  \noindent
  Le righe rappresentano i geni e le colonne le cellule. Ogni entry \( (gene, cell) \) 
  rappresenta il livello di espressione del gene in quella cellula. L'obiettivo è
  quello di creare un modello che raggruppa per similarità (clustering):
  \begin{itemize}
    \item cellule (colonne)
    \item geni (righe)
    \item entrambi
  \end{itemize}
  Non si sa il significato del raggruppamento, proprio perchè non si hanno etichette,
  però si sanno che i dati sono correlati.
\end{example}
Questo approccio può essere rappresentato da una funzione:
\[
  f(x)
\] 
Dato \( x \) si vuole "imparare" \( f() \) come una rappresentazione compatta di \( x \) 
(clustering).

\subsection{Reinforcement learning}
Consiste nell'addestrare un modello a partire da un insieme di dati di interazioni
con l'ambiente dando all'agente delle ricompense o penalità.
\begin{example}
  Consideriamo l'esempio di un agente che deve pianificare la traiettoria di un
  braccio robotico per prendere una pallina.
  \begin{itemize}
    \item Problema di controllo (sequential decision making)
    \item Segnale di reward per guidare le azioni dell'agente (ad esempio una ricompensa alta
      quando il braccio raggiunge la pallina)
    \item Richiede interazione con l'ambiente
    \item Tecniche statistiche guidate dai dati ma con focus sul controllo e non
...
\[
  y = f(x), z
\] 
in cui dato \( (x, z) \) si vuole "imparare" \( f() \) che genera \( y \). \( x \) è lo
stato corrente, \( z \) è il segnale di reward.
...
\[
  X = (X_1, X_2, \ldots, X_p)
\] 
si vuole predirre un output che è difficile da misurare:
\[
  Y = f(X) + \varepsilon 
\] 
dove \( \varepsilon  \) è un \textbf{errore casuale}, \textbf{indipendente} da \( X \)
e \( f() \) rappresenta un'informazione sistematica di \( X \) su \( Y \).
La \textbf{predizione} è la costruzione di un modello \( \hat{f}() \) che calcola:
\[
  \hat{Y} = \hat{f}(X) \text{ dato } X
\] 
\begin{example}
  \label{ex:educazione}
  Nel seguente esempio la superficie blu è il modello di predizione e si
  osserva che i valori (punti rossi) non cadono precisamente nel modello a causa
  dell'errore casuale \( \varepsilon  \):
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{esempio_predizione}
    \caption{Guadagno come funzione degli anni di educazione e anzianità}
  \end{figure}
\end{example}

\subsubsection{Accuratezza della predizione}
L'accuratezza di una predizione \( \hat{Y} \) di \( Y \) dipende da due quantità
(assumendo che \( \hat{f}() \) e \( X \) siano fissati):
\begin{itemize}
  \item \textbf{Errore riducibile}: dovuto all'errore nel costruire il modello
  \item \textbf{Errore irriducibile}: dovuto alla variabilità di \( \varepsilon  \) 
    (errore casuale indipendente da \( X \))
\end{itemize}
\[
  E(Y - \hat{Y})^2 
  = E \left[ f(X) + \varepsilon - \hat{f}(X) \right]^2
  = \underbrace{[f(X) - \hat{f}(X)]^2}_{\text{Errore riducibile}} +
  \underbrace{Var(\varepsilon)}_{\text{Errore irriducibile}}
\] 
Dove \( E(X) \) con \( X \) variabile casuale è il valore atteso della \textbf{media}
di \( X \). Ad esempio la media di una popolazione.
\( Var(X) \) è la \textbf{varianza} di \( X \), cioè la misura della dispersione
dei valori di \( X \) intorno alla media.

\subsubsection{Inferenza}
L'obiettivo è di capire come \( Y \) cambia al variare di \( X \) (non solo predire \( Y \)).
Bisogna sapere la forma di \( f() \). Alcune domande che ci si possono porre sono:
\begin{itemize}
  \item Quali variabili indipendenti sono associate alla risposta?
  \item Qual'è la relazione tra la risposta e ogni variabile indipendente?
  \item Si può approssimare \( Y \) con un modello lineare rispetto ad \( X \)?
\end{itemize}

\subsection{Stima di una funzione}
Per stimare una funzione \( f() \) consideriamo il seguente esempio:
\begin{example}
  Riprendiamo l'esempio \ref{ex:educazione} in cui vogliamo predirre il guadagno
  annuale di una persona in base agli anni di educazione e all'anzianità:
  \begin{itemize}
    \item Dati di training:
      \[
        n = 30 \text{ osservazioni}
      \] 
    \item Numero di predittori:
      \[
        p = 2
      \]
      \begin{itemize}
        \item Anni di educazione
        \item Anzianità
      \end{itemize}

    \item \( x_{ij} \) è il valore per l'osservazione \( i \) del predittore \( j \)

    \item \( y_i \) è il valore della risposta per l'osservazione \( i \)

    \item Il training set sono le coppie:
      \[
        (x_1, y_1), (x_2, y_2), \ldots, (x_n, y_n)
      \] 
      dove \( x_i = (x_{i1}, x_{i2}, \ldots, x_{ip})^T \) è il vettore dei predittori
      per l'osservazione \( i \).

    \item L'obiettivo è quello di trovare una stima \( \hat{f}() \) tale che:
      \[
...
\subsubsection{Metodi parametrici}
I metodi parametrici fanno delle assunzioni sulla forma funzionale di \( f() \) facendo
assunzioni sulla forma di \( f() \). È composta da due passi:
\begin{enumerate}
  \item Fare un assunzione sulla forma di \( f() \), ad esempio assumere che \( f() \) sia
    lineare
    \begin{itemize}
      \item \( f(X) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_p X_p \) con
        \( p + 1 \) parametri \( \beta_0, \ldots, \beta_p \) 
      \item Un modello è completamente identificato dai parametri
    \end{itemize}

  \item Trovare un procedimento per \textbf{fittare} o \textbf{trainare} il modello
    usando i dati di training
    \begin{itemize}
      \item Per un modello lineare bisogna stimare i parametri \( \beta_0, \ldots, \beta_p \).
        Bisogna quindi trovare i valori di \( \beta_j \) che minimizzano l'errore tali che:
        \[
          Y \approx \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_p X_p = \hat{f}(X)
        \] 
    \end{itemize}
\end{enumerate}
Il metodo più comune per trainare modelli lineari è il metodo dei \textbf{minimi quadrati}
(least square)
\begin{example}
  Riprendendo l'esempio \ref{ex:educazione}, possiamo creare un modello lineare:
  \[
    Income \approx \beta_0 + \beta_1 \cdot \text{ Years of education} + \beta_2 \cdot \text{ Seniority}
  \] 
  Si utilizza il metodo dei minimi quadrati per stimare i parametri \( \beta_0, \beta_1, \beta_2 \)
  e si ottiene:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{modello_lineare_guadagno}
    \caption{Modello lineare per il guadagno in base agli anni di educazione e all'anzianità}
  \end{figure}
\end{example}

\subsubsection{Metodi non parametrici}
Nei metodi parametrici non si hanno assunzioni su \( f() \) e si prova a far fittare
il più possibile \( f() \) ai dati di training.
\begin{itemize}
  \item \textbf{Vantaggi}:
    \begin{itemize}
      \item Può approssimare molto bene i dati di training
    \end{itemize}

  \item \textbf{Svantaggi}:
    \begin{itemize}
      \item Richiede molti dati di training per funzionare bene
      \item Può fare overfitting sui dati di training, cioè approssimare troppo bene i dati
        di training e non generalizzare bene su nuovi dati
    \end{itemize}
\end{itemize}
\begin{example}
  Riprendendo l'esempio \ref{ex:educazione}, possiamo utilizzare un metodo
  non parametrico per stimare \( f() \):
  \begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{metodo_non_parametrico_guadagno}
    \caption{Metodo non parametrico per il guadagno in base agli anni di educazione e all'anzianità}
  \end{figure}
  Che in confronto con la funzione originale usata per generare i dati di training si
  nota che è presente overfitting:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{metodo_non_parametrico_guadagno_2}
    \caption{Confronto tra funzione originale e metodo non parametrico}
  \end{figure}
\end{example}

\subsection{Accuratezza del modello}
\subsubsection{Interpretabilità e flessibilità}
Non esiste un modello migliore in assoluto, ogni modello ha i suoi vantaggi
e svantaggi misurati in base a:
\begin{itemize}
  \item \textbf{Interpretabilità}: quanto è facile capire come il modello fa le predizioni
  \item \textbf{Flessibilità}: quanto il modello può adattarsi a diverse forme di dati
\end{itemize}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{interpretabilita_flessibilita}
  \caption{Rappresentazione grafica di flessibilità vs interpretabilità per diversi metodi di statistical learning}
\end{figure}

\subsubsection{Capire l'accuratezza}
È importante capire che metodo utilizzare per un certo dataset. Alcuni modi per decidere
sono:
\begin{itemize}
  \item \textbf{Regressione}: Mean Squared Error (MSE)
    \[
      MSE = \frac{1}{n} \sum_{i=1}^{n} (y_i - \hat{f}(x_i))^2
    \] 
    dove \( y_i \) è il valore reale e \( \hat{f}(x_i) \) è la predizione
  \item \textbf{Training MSE}: MSE calcolato sui dati di training
  \item \textbf{Test MSE}: MSE stimato su nuovi dati non visti prima, è il parametro
    più importante per valutare l'accuratezza del modello
\end{itemize}
Un metodo che ha training MSE minimo non garantisce che abbia anche test MSE minimo:
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{training_test_mse}
  \caption{Sinistra: dati simulati dalla curva nera (cerchi); modello lineare (arancione); due smoothing splines (blu e verde). Destra:
    training MSE (curva grigia), test MSE (curva rossa), minimo possibile test MSE su tutti e tre i metodi (linea tratteggiata grigia). I quadrati
    rappresentano l'MSE raggiunto dai tre metodi}
\end{figure}

\subsubsection{Bias-variance tradeoff}
IL valore atteso del \textbf{test MSE} per un dato valore \( x_0 \) può essere sempre
scomposto in una somma di 3 componenti:
\begin{itemize}
  \item Varianza di \( \hat{f}(x_0) \): Quantità con cui \( \hat{f}(x_0) \) cambierebbe variando
    il dataset di training. Più flessibile è il metodo, più alta è la varianza

  \item Squared bias di \( \hat{f}(x_0) \): Errore introdotto dalle assunzioni fatte sul modello.
    Modelli più semplici (ad esempio lineari), hanno un bias più alto

  \item Varianza dell'errore casuale \( \varepsilon  \)
\end{itemize}
\[
  \color{red}E(y_0 - \hat{f}(x_0))^2\color{black} = Var(\hat{f}(x_0)) + [Bias(\hat{f}(x_0))]^2 + Var(\varepsilon)
\] 
\begin{figure}[H]
  \centering
  \includegraphics[width=0.3\textwidth]{bias_variance_tradeoff}
  \caption{Rappresentazione grafica del bias-variance tradeoff}
\end{figure}

\subsubsection{Stimare il test error (cross-validation)}
Per stimare il test error si usa l'approccio \textbf{Validation set} che consiste nel
dividere \textbf{casualmente} il dataset in due parti:
\begin{itemize}
  \item \textbf{Training set}: usato per addestrare il modello
  \item \textbf{Validation set} (o hold out set): usato per stimare il test error
\end{itemize}
Si fitta il modello sul training set e si calcola l'MSE sul validation set.
Il validation set \textbf{non è} il set test, quindi l'errore è un'approssimazione.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{validation_set}
  \caption{Rappresentazione grafica del validation set}
\end{figure}
...
\textbf{Svantaggi}:
\begin{itemize}
  \item La stima del test error può essere molto variabile a seconda di come
    viene diviso il dataset
  \item Si usano meno dati per addestrare il modello e quindi si può avere un
    errore stimato più alto
\end{itemize}

\subsubsection{Leave one out cross validation}
Per superare gli svantaggi del validation set si può utilizzare il metodo
\textbf{Leave one out cross validation} che consiste nel lasciare una sola osservazione
fuori dal training set e usare quella per il validation set. Si ripete questo
processo \( n \) volte e si fa la media dell'errore.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{leave_one_out_cross_validation}
  \caption{Rappresentazione grafica del leave one out cross validation}
\end{figure}
...
\textbf{Vantaggi:}
\begin{itemize}
  \item Si usano tutti i dati per addestrare il modello, quindi si ha meno bias e una
    stima del test error più accurata
  \item Non c'è nessuna casualità nella divisione del dataset, quindi se si ripete
    questo approccio \( N \) volte si ottiene sempre lo stesso risultato
\end{itemize}
Con questo approccio non si trovano i parametri ottimali del modello, ma si ottiene
il \textbf{tipo di modello} ottimale (ad esempio lineare, polinomiale, ecc...).
Successivamente si usano tutti i dati per addestrare il modello con il tipo
scelto.

\subsubsection{K-fold cross validation}
L'idea è quella di dividere l'insieme di osservazione in \( k \) gruppi (folds). 
Si usa un fold come validation set e gli altri \( k-1 \) folds come training set.
Si ripete questo processo \( k \) volte, cambiando il fold usato come validation
set. Si calcola l'errore medio sui \( k \) fold:
\[
  \{MSE_1, MSE_2, \ldots, MSE_3\} 
\] 
\[
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{k_fold_cross_validation}
  \caption{Leave one out cross validation come caso particolare di k-fold cross validation con \( k = n \)}
\end{figure}
...
\textbf{Vantaggi:}
\begin{itemize}
  \item Computazionalmente più efficiente del leave one out cross validation
    (LOOCV) per grandi dataset, solitamente si usa \( k = 5 \) o \( k = 10 \)
  \item Bias-varianza trade-off:
    \begin{itemize}
      \item Ogni split casuale delle osservazioni addestra un modello su meno
        osservazioni: bias più alto rispetto a LOOCV
      \item k-Fold media l'output di set di osservazioni meno correlate: varianza
        più bassa rispetto a LOOCV
    \end{itemize}
\end{itemize}
\begin{example}
  Il test MSE non può essere calcolato in un'applicazione reale, ma si può calcolare
  se si usano dati simulati. Nell'esempio seguente le curve di Cross Validation hanno
  la forma generale corretta, ma \textbf{sottovalutano} il valore vero del test MSE:
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{k_fold_cross_validation_example}
    \caption{Vero test MSE (blu), LOOCV (nero, tratteggiato), k-fold (arancione, \( k=10 \))}
  \end{figure}
\end{example}

\subsubsection{Visualizzazione intuitiva del bias-variance tradeoff}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{bias_variance_tradeoff_visualizzazione}
  \caption{Rappresentazione intuitiva del bias-variance tradeoff}
\end{figure}

\subsubsection{Confronto tra cross validation}
\begin{itemize}
  \item \textbf{Bias}:
    \begin{itemize}
      \item \textbf{Validation set} può sopravvalutare il test error perchè il
        training set utilizzato contiene soltanto metà delle osservazioni

      \item \textbf{LOOCV} fornisce una stima approssimativamente non distorta del test error
        dato che ogni training set contiene \( n - 1 \) osservazioni (quasi tante
        quante le osservazioni nell'intero dataset)

      \item \textbf{k-fold CV} (con \( k = 5 \) o \( k = 10 \)) porta a un livello intermedio di bias
        dato che ogni training set contiene \( \frac{(k-1)n}{k} \) osservazioni

      \item Per ridurre il bias si dovrebbe usare LOOCV.
    \end{itemize}

  \item \textbf{Varianza}:
    \begin{itemize}
      \item \textbf{LOOCV} media gli output di \( n \) modelli fittati, addestrati su
        un insieme di osservazioni quasi identico → gli output sono altamente
        (positivamente) correlati tra loro

      \item \textbf{k-fold CV} con \( k < n \), fa la media gli output di \( k \) modelli fittati
        meno correlati tra loro: la sovrapposizione tra i training set in ogni modello
        è più piccola

      \item \textbf{Bias-variance trade-off} è associato alla scelta di \( k \) in k-fold
        cross-validation

      \item Solitamente si usa \( k = 5 \) o \( k = 10 \) perchè questi valori
        hanno mostrato empiricamente di fornire stime del test error che non soffrono
        né di un bias eccessivamente alto né di una varianza molto alta
    \end{itemize}
\end{itemize}

\subsection{Regressione lineare}
La regressione lineare è un metodo utilizzato per predirre la risposta quantitativa
ed è un approccio basilare per il \textbf{supervised learning}.
I vantaggi sono:
\begin{itemize}
  \item Sono altamente interpretabili
  \item Introducono bassa varianza
\end{itemize}
Gli svantaggi sono:
\begin{itemize}
  \item Non sono abbastanza potenti per esprimere relazioni complesse tra le variabili
  \item Bassa accuratezza, alto bias
\end{itemize}

\subsubsection{Regressione lineare semplice (univariata)}
La funzione lineare univariata è una linea: $y = w_1 x + w_0$ dove:
\begin{itemize}
  \item \( w_0 \) è l'intercetta, cioè il valore di \( y \) quando \( x = 0 \)
  \item \( w_1 \) è il coefficiente angolare
\end{itemize}
Consideriamo il vettore dei \textbf{pesi}:
\[
  w = 
  \begin{pmatrix}
    w_0 \\
...
\] 
trovare il valore di \( w \) che rappresenta al meglio le osservazioni, cioè si vuole
trovare il vettore \( w \) che minimizza la \textbf{perdita}. Solitamente la perdita
è definita come la somma dei minimi quadrati degli errori (\( L_2 \) norm) su tutte le osservazioni
(Residual Sum of Squares oppure Least Squares)
\[
  Loss(h_w) = \sum_{i=1}^{n} L_2(y_i, h_w(x_i)) = \sum_{i=1}^{n} (y_i - h_w(x_i))^2
  = \sum_{i=1}^{n} (y_i - (w_1 x_i + w_0))^2
\] 

\begin{example}
  Consideriamo il seguente dataset che rappresenta le vendite di un prodotto in
  200 mercati diversi, insieme al budget pubblicitario per il prodotto in
  ciascuno di questi mercati per tre diversi media: TV, radio e giornale. Qui
  consideriamo solo la pubblicità in TV.
  \[
    sales = w_1 \cdot TV + w_0
  \] 
  dove:
  \begin{itemize}
    \item \( w_0 = 7.03 \)
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{regressione_lineare_semplice}
    \caption{Esempio di regressione lineare semplice}
  \end{figure}
\end{example}

\subsubsection{Trovare i pesi ottimali}
Per trovare i pesi ottimali \( w* \) si usa la seguente formula:
\[
  w* = argmin Loss(h_w) = argmin \sum_{i=1}^{n} (y_i - (w_1 x_i + w_0))^2
\] 
Questa funzione è minimizzata quando le derivate parziali rispetto a \( w_0 \)
e \( w_1 \) sono uguali a zero:
\[
  \begin{aligned}
    \frac{\partial Loss(h_w)}{\partial w_0} &= \frac{\partial}{\partial w_0} \sum_{i=1}^{n} (y_i - (w_1 x_i + w_0))^2 = 0\\
    \frac{\partial Loss(h_w)}{\partial w_1} &= \frac{\partial}{\partial w_1} \sum_{i=1}^{n} (y_i - (w_1 x_i + w_0))^2 = 0
  \end{aligned}
\] 
Queste equazioni hanno una sola soluzione che può essere calcolata come:
\[
  \begin{aligned}
    \hat{w}_1 &= \frac{n \left(\sum_{i=1}^{n} x_i y_i \right)
      - \left(\sum_{i=1}^{n} x_i\right)
    \left(\sum_{i=1}^{n} y_i\right)}
    {n \left(\sum_{i=1}^{n} x_i^2\right) - \left(\sum_{i=1}^{n} x_i\right)^2}\\
      \hat{w}_0 &= \frac{\left( \sum_{i=1}^{n} y_i \right) - \hat{w}_1
      \left( \sum_{i=1}^{n} x_i \right)}{n}
  \end{aligned}
\] 
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{spazio_pesi_regressione_lineare_semplice}
  \caption{Spazio dei pesi per la regressione lineare semplice con \( w = (\beta_0, \beta_1) \) }
\end{figure}
\noindent
Questo metodo \textbf{garantisce} che la funzione di loss sia convessa, quindi si ha sempre
un singolo punto di minimo e di conseguenza si trova sempre la soluzione ottimale. Questo
non è sempre garantito per altri modelli e si devono usare altri approcci come
il gradient descent.

\subsubsection{Gradient descent}
Il gradient descent è un algoritmo iterativo per trovare un \textbf{minimo locale}. L'algoritmo
è il seguente:
\[
  \left\{ \begin{aligned}
    &\text{inizializza vettore dei pesi \( w \) con un punto dello spazio dei pesi}\\
    &\text{Ripeti fino a convergenza:}\\
    &\quad w_i \leftarrow w_i - \alpha \frac{\partial}{\partial w_i} Loss(h_w) \;\forall w_i \in w\\
  \end{aligned} \right.
\] 
dove \( \alpha \) è il \textbf{learning rate} che controlla la dimensione del passo
ad ogni iterazione.
\begin{example}
  Per la regressione lineare semplice con loss \( L_2 \) abbiamo:
  \[
    \begin{aligned}
...
si ha solamente una sola \textbf{learning epoch}, cioè un solo passaggio attraverso
tutte le osservazioni. Il gradient descent stocastico consiste nel fare l'aggiornamento
dei pesi selezionando un piccolo sottoinsieme di osservazioni (minibatch) ad ogni
iterazione. Questo permette di calcolare l'aggiornamento in parallelo per ogni minibatch
rendendo l'altoritmo più veloce. Il problema di questo approccio è che la \textbf{convergenza
non è garantita}. La convergenza si potrebbe raggiungere con una procedura che riduce
\( \alpha \). Questo approccio è usato principalmente per funzioni loss non convesse e
nonostante non garantisca l'ottimalità ha buone performance.


\subsection{Reti neurali}
L'idea principale di una Artificial Neural Network (ANN) è di modellare una funzione
\textbf{non lineare} partendo da una combinazione lineare di input come feature derivate
(funzione applicata agli input).

\subsubsection{Feed-forward neural network}
Una rete neurale feed-forward è composta da un modello di regressione (o classificazione)
a due stadi rappresentato da un grafo aciclico diretto (DAG) in cui ogni nodo rappresenta
l'unità di computazione (neurone). Il modello è composto da:
\begin{itemize}
  \item \( X \) \textbf{Inputs}: \( \{ X_1, \ldots, X_p \} \)
  \item \( Z \) \textbf{Hidden layer(s)}: \( \{ Z_1, \ldots, Z_m \} \)
  \item \( Y \) \textbf{Output}: \( \{ Y_1, \ldots, Y_k \} \)
\end{itemize}
Gli archi rappresentano le connessioni tra i nodi e ad ogni arco è associato un peso
\( w_{ij} \) che rappresenta l'importanza della connessione tra il nodo \( i \) e il nodo
\( j \).
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{feed_forward_neural_network}
  \caption{Rappresentazione grafica di una rete neurale feed-forward}
\end{figure}
Un certo neurone (nodo) si \textbf{attiva} quando l'input \textbf{supera un certo limite}.
Ogni neurone calcola una \textbf{funzione di attivazione} (tipicamente non lineare) \( h \):
\[
  h(w^T x + b)
\] 
dove:
\begin{itemize}
  \item \( w \) è il vettore dei pesi associati agli archi in ingresso
  \item \( x \) è il vettore degli input
  \item \( b \) è il bias (intercetta)
\end{itemize}
Ogni layer ha la propria funzione di attivazione \( h \), solitamente:
\begin{itemize}
  \item \textbf{Hidden units}: \( \sigma \) che tipicamente è:
    \[
      \sigma  = \frac{1}{1 + e^{-x}} \text{ (sigmoide)}
    \] 

  \item \textbf{Output units}: \( g \) 
\end{itemize}

\subsubsection{Caolcolo di una rete neurale}
Il calcolo di una rete neurale si fa sommando tutti i pesi in ingresso moltiplicati
per gli output dei nodi di partenza. Questo valore viene passato alla funzione di
attivazione per calcolare l'output del nodo di arrivo.
\begin{itemize}
  ...
  \item \textbf{Output units}:
    ...
    \] 
  \item \textbf{In generale} gli output si calcolano come:
    \[
      y_k = h_2\left(\sum_{j} w_{kj}^{(2)} h_1\left(\sum_{i} w_{ji}^{(1)} x_i + b_j^{(1)}\right) + b_k^{(2)}\right)
    \] 
\end{itemize}
dove:
\begin{itemize}
  \item \( h_1, h_2 \) sono le funzioni di attivazione
  \item \( w^{(1)}, w^{(2)} \) sono i pesi associati agli archi
  \item \( b^{(1)}, b^{(2)} \) sono i bias, solitamente sono nodi costanti a 1
\end{itemize}
Un esempio di rete neurale feed-forward con 2 input, 2 hidden units e 1 output è:
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{feed_forward_neural_network_example}
  \caption{Esempio di rete neurale feed-forward}
\end{figure}
In una rete neurale ciò che si vuole stimare sono i pesi \( w \) associati agli archi.

\begin{example}
  Un esempio concreto di rete neurale feed-forward è il seguente:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{feed_forward_neural_network_example_2}
    \caption{Esempio di rete neurale feed-forward con funzioni di attivazione sigmoide}
  \end{figure}
\end{example}
...
\begin{itemize}
  \item \textbf{Threshold}: Questa funzione non è derivabile in \( x = 0 \), quindi
    non è usata nelle reti neurali moderne siccome non è possibile calcolare il gradient
    descent e quindi non è possibile stimare i pesi.
    \[
      h(x) =
      \begin{cases}
        1 & \text{se } x \ge 0\\
  ...
  \item \textbf{Sigmoid}:
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.4\textwidth]{sigmoid_function}
  ...
  \item \textbf{Tanh} (tangente iperbolica):
    \[
      h(x) = \tanh(x) = \frac{e^{x} - e^{-x}}{e^{x} + e^{-x}}
    \] 
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.4\textwidth]{tanh_function}
  ...
  \item \textbf{Gaussian}:
    \[
      h(x) = e^{-\frac{1}{2} \left( \frac{x - \mu}{\sigma} \right)^2}
    \] 
  ...
  \item \textbf{Rectified linear (ReLU)}: È la funzione di attivazione più usata nelle reti neurali moderne.
    \[
      h(x) = max \{0, x\} 
    \] 
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.4\textwidth]{relu_function}
    ...
    \vspace{1em}
    \noindent
    La funzione \textbf{softplus} è una versione smussata della ReLU (la approssima):
    \[
      h(x) = \log(1 + e^{x})
    \] 
    per \( x < 0 \) si ha \( h(x) \approx 0 \) e per \( x > 0 \) si ha \( h(x) \approx x \).
    La sua derivata prima è uguale alla funzione sigmoide ed è sempre compresa tra 0 e 1:
    \[
      \frac{dh(x)}{dx} = \frac{1}{1 + e^{-x}} = \sigma(x)
    \] 
\end{itemize}

\subsubsection{Proprietà universale di approssimazione}
\begin{theorem}[Hornik et al., 1989, Cybenko, 1989]
  Una rete neurale feed-forward con un layer di \textbf{output lineare} e \textbf{almeno un}
  hidden layer con una funzione di attivazione "squashing" (sigmoide/tanh/gaussian)
  \textbf{può approssimare qualsiasi funzione continua (definita su un insieme chiuso e
    limitato di \( \mathbb{R}^n \)) arbitrariamente bene}, a patto che la rete abbia un
    numero sufficiente di hidden units.
\end{theorem}

\subsubsection{Addestramento di una rete neurale}
L'addestramento di una rete neurale consiste nel trovare i pesi \( w \) basandosi
su un insieme di dati di training. L'insieme di pesi è composto da tutti i pesi
associati agli archi di ogni layer:
\[
   \begin{aligned}
     &\left\{ \alpha_{m0}, \alpha_{m}; m = 1, \ldots, M \right\}
     &\text{ \( M \cdot (p+1) \) pesi tra input e hidden layer}\\
     &\left\{ \beta_{k0}, \beta_{k}; k = 1, \ldots, K \right\}
     &\text{ \( K \cdot (M+1) \) pesi tra hidden layer e output}
   \end{aligned}
\] 
Per la \textbf{regressione}, cioè quando la risposta è quantitativa, si usa
la \textbf{somma dei minimi quadrati} come misura di fit:
\[
  RSS = \sum_{k=1}^{K} \sum_{i=1}^{N} \left( y_{ik} - f_k(x_i) \right)^2
\] 

\subsubsection{Back-propagation}
Siccome la rete neurale ha una forma composizionale, cioè la funzione di output
è una composizione di funzioni derivate dagli hidden layer, si può sfruttare questa
struttura per calcolare efficientemente le derivate
della funzione di loss rispetto ai pesi usando l'algoritmo di \textbf{Back-propagation}:
\begin{itemize}
  \item Usare i pesi correnti per calcolare \( f_k(x_i) \), calcolo in avanti
    (forward computation) sulla rete

  \item Eseguire una fase all'indietro (backward phase) calcolando gli errori
    nell'ultimo layer e propagando l'errore al layer precedente

  \item Aggiornare i pesi usando il gradient descent
\end{itemize}
\[
  E(W) = \sum_{i=1}^{N} E_i = \sum_{i=1}^{N} \sum_{k=1}^{K} \left( y_{ik} - f_k(x_i) \right)^2
\] 
Quindi considerando un hidden unit \( z_mi \):
\[
  z_mi = \sigma \left( \alpha_{m0} + \alpha_{m}^T x_i \right) \quad z_i = \{ z_{1i}, \ldots, z_{Mi} \}
\] 
Si può calcolare la derivata parziale dell'errore rispetto ai pesi come:
\[
  \begin{aligned}
    &\frac{\partial E_i}{\partial \beta_{km}} = \color{blue}-2 \left( y_{ik} - f_k(x_i) \right) \cdot
    g'_k \left( \beta_{k}^T z_i \right)\color{black} \cdot z_{mi}\\
    &\frac{\partial E_i}{\partial \alpha_{ml}} = \sum_{k=1}^{K} \color{blue}-2
    \left( y_{ik} - f_k(x_i) \right) \cdot g'_k \left( \beta_{k}^T z_i \right)\color{black} \cdot
    \beta_{km} \cdot \sigma' \left( \alpha_{m}^T x_i \right) \cdot x_{il}
  \end{aligned}
\] 
La parte in blu rappresenta l'errore propagato all'indietro (back propagation).
Successivamente si aggiornano i pesi usando il gradient descent all'iterazione \( (r+1) \):
\[
  \begin{aligned}
    &\beta_{km}^{(r+1)} = \beta_{km}^{(r)} - \gamma_r \sum_{i=1}^{N} \frac{\partial E_i}{\partial \beta_{km}^{(r)}}\\
    &\alpha_{ml}^{(r+1)} = \alpha_{ml}^{(r)} - \gamma_r \sum_{i=1}^{N} \frac{\partial E_i}{\partial \alpha_{ml}^{(r)}}
  \end{aligned}
\] 

\vspace{1em}
\noindent
Il back propagation è molto semplice e \textbf{locale}, cioè ogni unità calcola la
propria parte di errore e la invia alle unità collegate a essa, quindi è facilmente
parallelizzabile. Il training può essere effettuato in due modi:
\begin{itemize}
  \item \textbf{Batch}: Un solo aggiornamento dei pesi sommando i gradienti dell'errore
    su tutte le osservazioni.

  \item \textbf{Online}: Aggiornamento dei pesi per ogni osservazione.
\end{itemize}
\( \gamma_r \) è la costante di \textbf{batch learning} che dovrebbe diminuire a 0
per \( r \to \infty \). Il valore che garantisce la convergenza è:
\[
  \gamma_r = \frac{1}{r}
\] 

\subsubsection{Deep neural networks}
Le reti neurali profonde (DNN) sono reti neurali con più di un hidden layer. Il vantaggio
è quello di avere espressività maggiore (più compatta rispetto ad un layer con tanti nodi),
ma i problemi da affrontare sono: 
\begin{itemize}
  \item Addestrare la rete neurale (back-propagation diventa più complesso)
  \item Evitare l'overfitting (regolarizzazione più complessa)
\end{itemize}
Il problema principale delle deep neural networks (con funzioni "squashing") è il
\textbf{vanishing gradient}, cioè durante il back-propagation i gradienti
diventano sempre più piccoli man mano che si procede verso gli strati più bassi della rete.
Questo avviene perchè le funzioni "squashing" hanno derivate comprese tra 0 e 1, quindi
moltiplicando più derivate si ottiene un valore sempre più piccolo. Questo è il motivo
per cui si usano funzioni di attivazione come la ReLU che non soffrono di questo problema.
\begin{example}
  Prendiamo in considerazione la funzione \( tanh \) e osserviamo la derivata:
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{tanh_vanishing_gradient}
    \caption{Derivata della funzione \( tanh \)}
  \end{figure}
  Questo avviene anche per la funzione \textbf{softplus} che anch'essa ha derivata
  minore di 1 nonostante sia un'approssimazione della ReLU.
\end{example}

\begin{example}
  Consideriamo un dataset di immagini con label che indicano la linea d'acqua, l'addestramento
  della rete neurale consiste nel trovare i pesi che riescono a dare in output un pixel
  nero dove c'è acqua e un pixel bianco dove non c'è acqua:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{esempio_training}
    \caption{Esempio di addestramento di una rete neurale per il riconoscimento della linea d'acqua}
  \end{figure}
\end{example}

\subsection{Generative AI}
Generative AI è un campo dell'intelligenza artificiale che si concentra sulla
creazione di modelli in grado di generare nuovi dati simili a quelli su cui sono
stati addestrati. L'idea principale è quella di rappresentare i concetti come
vettori (\textbf{embedding}) in uno spazio multidimensionale, in modo che
le relazioni tra i concetti possano essere catturate attraverso operazioni
aritmetiche sui vettori. I concetti simili sono vicini nello spazio degli embedding.
\begin{example}
  Un esempio di word embedding, con alcune operazioni sui vettori è il seguente:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{word_embedding_example}
    \caption{Esempio di word embedding}
  \end{figure}
\end{example}

\subsubsection{Rappresentare i concetti tramite embedding}
Per trovare la corretta rappresentazione nello spazio degli embedding dei concetti
si usa la \textbf{contrastive learning}, che consiste nel dare esempi positivi e
negativi al modello. Gli esempi positivi sono coppie di concetti simili, mentre
gli esempi negativi sono coppie di concetti non simili. I concetti possono essere di
diversi tipi, come immagini, testo, audio, video, ecc...
\begin{example}
  Un esempio di contrastive learning è il seguente:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{contrastive_learning_example}
    \caption{Esempio di contrastive learning per immagini}
  \end{figure}
\end{example}

\subsubsection{Attenzione}
L'importanza di un conncetto dipende dal contesto in cui viene usato. Ad esempio:
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{attention_example}
  \caption{Esempio di attenzione nei modelli di linguaggio}
\end{figure}
Questo meccanismo è chiamato \textbf{attenzione}.

\vspace{1em}
\noindent
I \textbf{transformer} sono un'architettura di rete neurale basata sul meccanismo di
attenzione. L'idea principale è quella di permettere al modello di "prestare attenzione"
a diverse parti dell'input in base al contesto.

\subsubsection{Language models}
I \textbf{Language Models} (LM) sono modelli che trasformano i concetti in
embedding di testo in modo da predirre la parola successiva in una frase.
Il modello si basa su:
\begin{itemize}
  \item Embedding e tokenizzazione per rappresentare l'input
  \item Transformer, attention per rappresentare il contesto
  \item \textbf{Self-supervised learning} per addestrare il modello su grandi quantità di testo
    senza bisogno di label
\end{itemize}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{language_model}
  \caption{Rappresentazione grafica di un language model}
\end{figure}
Un Large Language Model (LLM) è un LM con un numero molto grande di parametri,
almeno 10 miliardi di parametri.

\subsection{Reinforcement learning}
Il \textbf{Reinforcement Learning} (RL) è un paradigma di apprendimento automatico
in cui un agente apprende a prendere decisioni ottimali, massimizzando una ricompensa,
interagendo con un ambiente.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{reinforcement_learning}
  \caption{Rappresentazione grafica del reinforcement learning}
\end{figure}
\noindent
I concetti chiave del RL sono:
\begin{itemize}
  \item \textbf{Trial and error}: l'agente esplora l'ambiente e apprende dalle conseguenze delle sue azioni
  \item \textbf{Delayed reward}: le azioni possono avere conseguenze a lungo termine
\end{itemize}
L'obiettivo del RL è stimare a lungo termine la \textbf{funzione di valore} \( V(s) \),
e trovare la \textbf{policy} ottimale \( \pi(s) \) che massimizza la ricompensa attesa.

\vspace{1em}
\noindent
Il reinforcement learning è un MDP (Markov Decision Process), ma:
\begin{itemize}
  \item Non si conoscono gli stati positivi o negativi (non si conosce \( R(s, a, s') \))
  \item Non si conoscono le transizioni tra stati (non si conosce \( T(s, a, s') \))
\end{itemize}
...
\begin{itemize}
  \item \textbf{Model-based}: sono metodi che cercano di \textbf{stimare il modello
    dell'ambiente}. Questi metodi evitano stati e azioni che portano a ricompense negative
    e quindi usano meno step per raggiungere l'obiettivo con un utilizzo più efficiente
    dei dati.

  \item \textbf{Model-free}: sono metodi cercano di \textbf{imparare la Q-function
    e la policy ottimale} direttamente dalle esperienze dell'agente. Questi metodi
    sono più semplici da implementare perchè non richiedono un modello, ma hanno
    bisogno di più dati per apprendere una buona policy. Una diretta conseguenza
    è che questi metodi hanno meno bias rispetto ai metodi model-based.
\end{itemize}
\begin{example}
  Si vuole calcolare l'età attesa per una classe data la distribuzione di
  probabilità:
  \[
    \mathbb{E}[A] = \sum_a P(a) \cdot a
  \] 
  \begin{itemize}
    \item \textbf{Model-based}: stimare \( \hat{P}(a) \):
      \[
      ...
      \vspace{1em}
      \noindent
       Oltre a stimare il modello il duale è la stima della funzione
       di transizione \( T(s, a, s') \) che si può calcolare dati i samples
       con le relative azioni (traiettoria):
       \[
         s_0, a_0, s_1, a_1, s_2, a_2, \ldots
       \] 
       Si stima \( \hat{T}(s, a, s') \):
       \[
       \hat{T}(s, a, s') = \frac{count(s_{t+1} = s', a_t = a, s_t = s)}{count(s_t = s, a_t = a)}
       \] 
       Lo stesso vale per la stima della funzione di ricompensa

    \item \textbf{Model-free}: campionare \( N \) studenti e calcolare la media delle età:
      \[
...
\begin{lstlisting}
Require: A, S, S_0
Ensure: T_hat, R_hat, pi_hat
  Initialize T_hat, R_hat, pi_hat
  repeat
    Execute pi_hat for a learning episode
    Acquire a sequence of tuples <(s, a, s', r )>
    Update T_hat and R_hat according to tuples <(s, a, s', r )>
    Given current dynamics compute a policy (e.g., VI or PI)
  until termination condition is met
\end{lstlisting}

\subsubsection{Metodi model-free}
L'obiettivo dei metodi model-free è quello di stimare un'expectation pesata
per il modello senza il modello, quindi partendo dai campioni:
\[
  x_i \sim P(x),\; \mathbb{E}[f(x)] \approx \frac{1}{N} \sum_{i} f(x_i)
\] 
Dove \( x_i \sim P(x) \) indica che i campioni sono estratti dalla distribuzione
di probabilità \( P(x) \). Alla fine si vuole stimare una funzione di valore
data la policy \( \pi \):
\begin{itemize}
  \item Si esegue \( \pi \) per alcuni episodi di learning
  \item Si calcola la somma dei reward scontati \textbf{ogni volta che si visita uno stato}
  \item Si calcola la media dei reward scontati per ogni campione
\end{itemize}

\subsubsection{Sample-based policy evalueation}
L'obiettivo è quello di \textbf{migliorare} la funzione di valore \( V \) considerando
la Bellman Update, data una policy \( \pi  \):
\[
  V_{\pi}^{k+1}(s) = \sum_{s'} T(s, \pi(s), s') \left( R(s, \pi(s), s') + \gamma V_{\pi}^{k}(s') \right)
\] 

\subsubsection{Temporal Difference Learning}
Il Temporal Difference Learning permette di usare la Sample-based policy evaluation
per \textbf{imparare da ogni esperienza} senza aspettare la fine dell'episodio.
Questo viene fatto calcolando una \textbf{running average}:
\begin{itemize}
  \item Si parte dal campione:
    \[
      sample = R(s, \pi(s), s') + \gamma V_{\pi}(s')
    \] 

  \item Si aggiorna il valore come:
    \[
      V_{\pi}(s) = (1 - \alpha) V_{\pi}(s) + \alpha (sample - V_{\pi}(s))
    \] 

  \item \( \alpha \) deve decrescere nel tempo per garantire la convergenza,
    l'opzione più semplice è mettere \( \alpha = \frac{1}{n} \) dove \( n \)
    è il numero di volte che lo stato \( s \) è stato visitato.
\end{itemize}
Quindi:
\[
...
\vspace{1em}
\noindent
L'obiettivo è calcolare una policy, ma questo non si può fare perchè per calcolarla bisognerebbe
trovare l'azione che massimizza \( Q(s,a) \):
\[
  \pi(x) = argmax_a Q(s,a)
\] 
dove:
...
\] 
Non si può usare direttamente \( V \) per calcolare \( \pi \).
L'idea è quella di stimare direttamente i valori di \( Q(s,a) \).
...
\subsubsection{SARSA: alternativa on-policy al Q-learning}
Questo algoritmo è simile al Q-learning, ma segue la policy corrente per scegliere
l'azione successiva \( a' \) invece di scegliere l'azione che massimizza \( Q(s', a') \).
Il Q-value viene fatto sapendo quale azione si prenderà nel prossimo stato.
\begin{lstlisting}
Initialize Q(s, a), for all S, a in A(s), arbitrarily, and Q(terminal-state, *) = 0
  Repeat (for each episode):
    Initialize s
    ...
    until s is terminal
\end{lstlisting}
Questo algoritmo converge solo se la policy usata per scegliere le azioni converge
alla policy greedy e ogni coppia stato-azione viene visitata infinite volte.
...
\subsubsection{Funzioni di esplorazione}
L'idea è quella di aggiungere un bonus di esplorazione alla ricompensa
per incentivare l'agente a esplorare nuove azioni. Si esplorano le aree in cui non
si è sicuri che siano negative (optimism in face of uncertainty). La funzione di esplorazione
è definita considerando:
\begin{itemize}
  \item Una stima \( u \)
  \item Un numero di visite \( n \)
  \item Il calcolo di una funzione \( f(u, n) = u + \frac{k}{n} \)
\end{itemize}
\begin{itemize}
  \item Update regolare della Q-function:
    \[
      Q(s,a) = (1 - \alpha) Q(s,a) + \alpha (R(s, a, s') + \gamma \max_{a'} Q(s', a'))
    \]

  \item Update modificato con bonus di esplorazione:
    \[
      Q(s,a) = (1 - \alpha) Q(s,a) + \alpha (R(s, a, s') + \gamma \max_{a'} f(Q(s', a'), N(s, a)))
    \]
    dove:
    \begin{itemize}
      \item \( N(s', a') \) è il numero \( n \) di volte che una coppia stato-azione
        \( (s', a') \) è stata visitata
      \item \( f(Q(s', a'), N(s, a)) \) è la funzione di esplorazione che aggiunge
        un bonus alla stima di \( Q(s', a') \) in base a \( N(s, a) \)
      \item \( k \) è un parametro fissato
    \end{itemize}
\end{itemize}

\subsection{Deep reinforcement learning}
Nel mondo reale gli stati e le azioni sono spesso continui e in grandi quantità e
quindi non si riesce a rappresentare le funzioni chiave per il reinforcement learning:
\( (\pi(s), V(s), Q(s,a)) \) come tabelle. Per risolvere questo problema si possono
approssimare usando:
\begin{itemize}
  \item Approssimazione lineare
  \item Approssimazione tramite rete neurale (Deep Reinforcement Learning)
  \item Approssimazione con Deep Q-Networks (DQN) in cui si approssima \( Q(s,a) \)
    usando una Deep Neural Network
\end{itemize}

\subsubsection{Gradient Q-Learning}
Per approssimare \( Q(s,a) \) con una funzione parametrica \( Q_w(s,a) \) si può
usare il gradiente per aggiornare i pesi \( w \):
\begin{itemize}
  \item Stima \( Q_w(s,a) \) 
  \item Target \( r(s,a,s') + \gamma \max_{a'} Q_{\overline{w}}(s', a') \), cioè il valore
    che si otterrebbe se si conoscesse il modello
\end{itemize}
I pesi si stipano minimizzando una funzione di errore, in questo caso l'errore quadratico:
\[
  Err(w) = \left( \underbrace{Q_w(s,a)}_{\text{Stima}}
  - 
  \underbrace{r(s,a,s') - \gamma \max_{a'} Q_{\overline{w}}(s', a')}_{\text{Valore reale}} \right)^2
\] 
per farlo si usa il gradiente:
\[
  \frac{\partial Err(w)}{\partial w} = 2 \left( Q_w(s,a) - r(s,a,s') - \gamma \max_{a'} Q_{\overline{w}}(s', a') \right)
  \cdot \frac{\partial Q_w(s,a)}{\partial w}
\] 
Lo scalare 2 non è rilevante per l'aggiornamento perchè l'unica cosa che conta è la direzione
del gradiente.
...
\end{lstlisting}
Si nota che non si usa \( \overline{w} \) per calcolare il gradiente, ma si usa la stima
perchè il valore reale non è noto. Per questo motivo \textbf{non c'è garanzia di convergenza}.
La convervenza è garantita solo se si usa il \textbf{Linear gradient Q-learning}.


\subsection{Mitigare la divergenza}
Ci sono due approcci principali per mitigare la divergenza nel Deep Q-learning:
\begin{itemize}
  \item Experience replay
  \item Utilizzare due network diverse:
    \begin{itemize}
      \item Q-network
      \item Target network
    \end{itemize}
...
L'idea è quella di memorizzare in un buffer le esperienze dell'agente (ad esempio \( (s, a, s', r) \))
e usarle ad ogni step di apprendimento. Ad ogni passo si estrae un batch casuale di esperienze
dal buffer e si usa per aggiornare i pesi della rete neurale. I vantaggi sono:

\begin{itemize}
  \item Riduce la correlazione tra campioni successivi (aumenta la stabilità)
  \item Riduce il numero di interazioni con l'ambiente (aumenta l'efficienza dei dati)
\end{itemize}

\subsubsection{Target network}
L'idea è quella di mantenere una rete \textbf{target} separata da aggiornare periodicamente,
ma non ad ogni step.

\begin{itemize}
  \item Il Q-network calcola: \( Q_w(s,a) \)
  \item Il Target network calcola: \( Q_{\overline{w}}(s', a') \)
\end{itemize}

\subsubsection{Deep Q Network}
Il Deep Q Network (DQN) è un algoritmo di Deep Reinforcement Learning che
utilizza una rete neurale profonda per approssimare la Q-function \( Q(s ,a) \).
Si utilizzano sia l'experience replay che il target network per migliorare
la stabilità e l'efficienza dell'apprendimento. Lo pseudocodice del DQN è il seguente:
...
loop
  ...
  for (s_hat, a_hat, s', r_hat) in MiniBatch do
    grad = (Q_w(s_hat, a_hat) - r_hat - gamma * max_a_hat'(Q_w_hat(s_hat', a_hat'))) * partial Q_w(s_hat, a_hat)/partial w
